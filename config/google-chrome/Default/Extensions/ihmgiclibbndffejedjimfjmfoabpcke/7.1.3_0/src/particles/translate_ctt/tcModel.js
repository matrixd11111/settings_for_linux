    ...
    pl.extend(ke.particles.translate_ctt.model, {
        ...
        display: function (is_offline, data, manual_show) {
            ...
            ke.ext.util.storageUtil.chainRequestBackgroundOption([
                ...
                if (data.rHref) {
                    $tt.find('.' + ke.getPrefix() + 'iw-' + data.old_data.id + '.' + ke.getPrefix() + 'hide').removeClass(ke.getPrefix() + 'hide').find('a').attr('href', data.rHref);
                    with_info_wrap = true;
                } else {
                    $tt.find('.' + ke.getPrefix() + 'iw-' + data.old_data.id + ':not(.' + ke.getPrefix() + 'hide)').addClass(ke.getPrefix() + 'hide').find('a').removeAttr('href');
                }
                ...
                if (ke.IS_SAMSUNG) {
                    $tt.find('.' + ke.getPrefix() + 'unpin').hide();
                }

                if (sett_shows < 5) {
                    ke.ext.util.storageUtil.requestBackgroundOption('setVal', ['new_settings_counter', sett_shows + 1]);
                } else {
                    $tt.find('.' + ke.getPrefix() + 'new-badge').remove();
                }
            ...
            if (data.rHref) {
                ke.app.render.events.localizedSiteInfoWarnActions(data.old_data.id);
            }
