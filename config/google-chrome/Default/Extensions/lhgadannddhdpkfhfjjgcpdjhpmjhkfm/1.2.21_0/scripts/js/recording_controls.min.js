...
$(document).ready(function () {
  ChromeRuntimeAPI.get_background_context().then(function (context) {
    ...
    var ms_rec_controls = new MSScreencastRecordingControlsView();
  });
});
...
function MSScreencastRecordingControlsView() {
  ...
  function initialize() {
    _dependencies.transmitter.subscribe({
      'update_memory_status': function update_memory_status(request, callback) {
        return ui.update_memory_status(request.memory.used, request.memory.available);
      },
      'update_timer': function update_timer(request, callback) {
        return ui.update_timer(request.duration);
      },
      'refreshrecordui': function refreshrecordui(request, callback) {
      ...
      'change_audio_control_view': function change_audio_control_view(request, callback) {
        if (request.show) {
  ...
  var ui = {
    ...
    assign_event_handlers: function assign_event_handlers() {
          ...
          if (navigator.getUserMedia) {
            navigator.getUserMedia({
              ...
              hardware.media_devices.get_microphones(function (devices) {
                ...
                _dependencies.transmitter.broadcast('update_audio_input', {
                  audio: ui.devices[0]
                });
              });
            }, function () {
      ...
      $(ui.$elements.permHelper).on('click', function (e) {
        ...
        chrome.windows.getAll(function (data) {
          var w = data.filter(function (item) {
      ...
      $(document).on('change', ui.$elements.audioInput, function (e) {
        ...
        _dependencies.transmitter.broadcast('update_audio_input', send);
      });
    ...
    update_timer: function update_timer(duration) {
      ui.$elements.record_duration.text(convert_seconds_to_time_string(duration));
    },
    update_memory_status: function update_memory_status() {
      var used = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 0;
      var free = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 0;
  ...
  var record = {
    ...
    audio: {
      enabled: false,
      toggle: function toggle(record_audio) {
        record_audio = record_audio === true;

        _dependencies.transmitter.broadcast('record_desktop', {
          action: new_value === true ? 'enable_audio' : 'disable_audio'
        }).then(function () {
          return record.audio.enabled = record_audio;
        });
      }
    },
    start: function start() {
      return _dependencies.transmitter.broadcast('record_desktop', {
        action: 'start'
