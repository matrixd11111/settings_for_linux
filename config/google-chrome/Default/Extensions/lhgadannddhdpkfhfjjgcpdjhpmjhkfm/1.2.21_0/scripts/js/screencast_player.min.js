...
var page_type = 'screencast_player';
var bg;
var session_id, session;
var ms_screencast_player;
$(document).ready(function () {
...
function MSScreencastPlayerView() {
  ...
  self.state = {
    ...
    history: {
      ...
      wipe: function wipe() {
        ...
        self.state.history.handle_update(); // _scene.fit_image_to_window(session.data.original);
      }
    }
  };
  self.ui = {
    ...
    assign_event_handlers: function assign_event_handlers() {
      $(document).on('output_type_changed', function (e, data) {
        console.log("[Event] Export format selected: ".concat(data.type));

        if (self.ready) {
          var current_position = self.video.element.currentTime;
      ...
      self.ui.$elements.play_button.on('click', self.video.toggle_playback); // GIF edit
      ...
      $(document).on('mousemove', function (e) {
        var canvas = self.ui.$elements.crop_overlay;
        var canvas_position = $(canvas).position();
        var canvas_offset = $(canvas).offset();
        var current_point = new Point((e.pageX - canvas_offset.left).clamp(0, $(canvas).width()), (e.pageY - canvas_offset.top).clamp(0, $(canvas).height()));
        var primitive = self.state.current_primitive;
              ...
              if (self.state.draw) {
                ...
                switch (primitive.context) {
                  ...
                  case 'move':
                    ...
                    var top_left_corner = new Point((primitive.x + area_offset.x).clamp(0, primitive.boundaries.x), (primitive.y + area_offset.y).clamp(0, primitive.boundaries.y));
                    primitive.points = [top_left_corner, new Point(top_left_corner.x + primitive.width, top_left_corner.y + primitive.height)];
                  ...
                  case 'resize':
                    ...
                    primitive.points = [new Point(current_point.x, current_point.y), new Point(primitive.handles[opposite_corners[primitive.active_handle]].x, primitive.handles[opposite_corners[primitive.active_handle]].y)];
                    break;
                }
                ...
                primitive.x <= current_point.x && current_point.x <= primitive.x + primitive.width && primitive.y <= current_point.y && current_point.y <= primitive.y + primitive.height) {
                  primitive.context = 'move';
                  canvas.style.cursor = 'move';
                } // cursor is above one of the handles


                for (var direction in primitive.handles) {
                  var handle = primitive.handles[direction];

                  if (Math.pow(handle.x - current_point.x, 2) + Math.pow(handle.y - current_point.y, 2) <= Math.pow(5, 2)) {
                    canvas.style.cursor = "".concat(direction, "-resize");
      ...
      $(self.ui.$elements.crop_overlay).on('mouseenter', function (e) {
        if (!self.state.draw) {// self.ui.$elements.crop_selection_hint.show();
        }
      });
      $(self.ui.$elements.crop_overlay).on('mouseout', function (e) {// self.ui.$elements.crop_selection_hint.hide();
      });
      $(document).on('click', '#crop_submit_button', self.tools.crop.submit);
      $(document).on('click', '#crop_cancel_button', self.tools.unselect); // Resize UI
    ...
    fit_video_to_window: function fit_video_to_window(width, height) {
    ...
    resize_page: function resize_page() {
  ...
  self.tools = {
    ...
    crop: {
      ...
      resize_underlay: function resize_underlay() {
      ...
      update: function update(x, y, width, height) {
        self.state.current_primitive.points = [new Point(x, y), new Point(x + width, y + height)]; // console.log(x, y, width, height);

        self.state.current_primitive.draw();
      },
      submit: function submit() {
        var scale = $(self.ui.$elements.media_player).width() / self.ui.$elements.media_player.width; // console.log(selection, scale);
    ...
    resize: {
      ...
      set_new_image_size: function set_new_image_size(input_data, side, keep_proportion) {
        ...
        if (value) {
          ...
          side = side || 'width';
          var other_side = side === 'width' ? 'height' : 'width'; // console.log(value, value/self.video[side]);
      ...
      render_preview: function render_preview() {// Nothing at this time. Come back later, stranger!
      },
      submit: function submit() {
        var primitive = self.state.current_primitive;
    ...
    finish_primitive_creation: function finish_primitive_creation() {
      ...
      if (['crop', 'resize'].has(self.state.current_primitive.type)) {// Fit video to window
      }
  ...
  self.video = {
    ...
    initialize: function initialize() {
      ...
      self.video.element.onseeking = function () {
        console.log("[Event] Seeking to ".concat(this.currentTime));
      };

      self.video.element.onseeked = function () {
        console.log("[Event] Seeked to ".concat(convert_seconds_to_time_string(this.currentTime), ":").concat(convert_seconds_to_time_string(self.duration)));

        if (!self.video.duration_read) {
          console.log("2. Now we know that duration is ".concat(this.currentTime));
          self.video.duration_read = true;
          self.duration = session.duration = this.currentTime;
          this.currentTime = 0;
        } else {// Already is in oncanplaythrough event listener
          // self.video.update_navigation();
        }
      };

      self.video.element.onloadeddata = function () {
        console.log("[Event] Data loaded (ready:".concat(self.ready, ")"));
      };

      self.video.element.oncanplaythrough = function () {
        console.log("[Event] Can play through\n Duration calculated: ".concat(self.video.duration_read, "\n Player is ready: ").concat(self.ready));
        ...
        if (!self.ready) {
          if (!self.video.duration_read) {
            ...
            self.ui.$elements.playback_slider = new MSMediaPlaybackSlider({
              ...
              on_boundary_change: function on_boundary_change(begin, end) {
                session.begin = begin;
                session.end = end;
              },
              on_navigate: function on_navigate(playback_position) {
    ...
    update: function update() {
      ...
      self.ui.$elements.video_dimensions.html("".concat(self.video.width, "&nbsp;<span style=\"font-size:14px;\">&Cross;</span>&nbsp;").concat(self.video.height));
      self.video.render_current_frame();
    },
    render: function render() {
      if (self.video.element.paused || self.video.element.ended) {
        return false;
      }
    ...
    toggle_playback: function toggle_playback() {
      if (self.ready) {
        console.log("Video paused: ".concat(self.video.element.paused));

        if (self.video.element.paused) {
          console.log('[Event] Starts playing a video');
          self.video.element.play().then(function () {
            return console.log('Promise resolved');
          })["catch"](function (e) {
            return console.log('[!] Pause interrupted video playback', e);
          });
        } else {
...
function Crop(points) {
  ...
  this.draw = function (canvas) {
    ...
    var context = canvas.getContext('2d');
    this.update(); // this.border_dash_offset += (this.border_dash_offset < 11) ? 1 : 0;
    ...
    for (var direction in this.handles) {
