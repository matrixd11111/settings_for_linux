...
function MSToolSizeSlider() {
  ...
  var ui = {
    ...
    assign_event_handlers: function assign_event_handlers() {
      ui.$elements.wrapper.on('click', convert_mouse_position_to_value).on('mousedown', function () {
        _dragging = true;
      });
      $(document).on('mousemove', function (e) {
        if (_dragging) {
          convert_mouse_position_to_value(e);
        }
      }).on('mouseup', function () {
        _dragging = false;
      });
      ...
      function convert_mouse_position_to_value(e) {
        var slider_width = ui.$elements.wrapper.width();
        var mouse_x_relative = e.pageX - ui.$elements.wrapper.offset().left;
  ...
  function set_value(value) {
    if (typeof value === 'number') {
      ...
      if (typeof _callbacks.on_change === 'function') {
        _callbacks.on_change(_value);
      }
    }
  }
...
function MSMediaPlaybackSlider() {
  var parameters = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
  var self = this;

  var _duration = parameters.duration || 1;

  var _value = parameters.value || 0;

  var _step = parameters.step || 0.1;
  ...
  var ui = {
    ...
    initialize: function initialize(parameters) {
      ...
      ui.$elements.trim_handles = {
        ...
        begin: ui.$elements.wrapper.find("".concat(trim_handle_selector, "[data-position=begin]")),
        end: ui.$elements.wrapper.find("".concat(trim_handle_selector, "[data-position=end]"))
      };
      ui.assign_event_handlers();
    },
    assign_event_handlers: function assign_event_handlers() {
      ui.$elements.wrapper.on('click', set_new_playback_position).on('mousedown', function () {
        self.dragging = true;
      });
      ui.$elements.trim_handles.both.on('mousedown', function (e) {
        e.stopPropagation();
    ...
    render_playback_position: function render_playback_position() {
    ...
    render_playback_interval: function render_playback_interval() {
  ...
  ;
  var _export_interval = {
    begin: 0,
    end: 0,
    duration: 0,
    dragging: false,
    set_boundary: function set_boundary(time, position) {
      var min_interval = 0.5;
      _export_interval[position] = position === 'begin' ? time.clamp(0, _export_interval.end - min_interval) : time.clamp(_export_interval.begin + min_interval, _duration);
  ...
  var _playback = {
    set_current_position: function set_current_position(new_value) {
      _value = new_value.clamp(_export_interval.begin, _export_interval.end);
      ui.render_playback_position();
    },
    navigate_to: function navigate_to(value) {
      _playback.set_current_position(value); // console.log(value, _value);
...
MSColorTools.convert = {
  hex_to_rgb: function hex_to_rgb(hex_string) {
    if (hex_string[0] === '#') {
    ...
    ['r', 'g', 'b'].forEach(function (channel, i) {
      return color[channel] = parseInt(hex_string.slice(2 * i, 2 * i + 2), 16);
    });
    return color;
  },
  rgb_to_hex: function rgb_to_hex(color_object) {
    return '#' + ['r', 'g', 'b'].map(function (channel) {
      return color_object[channel].toString(16).add_leading_symbols('0', 2);
    }).join('');
  },
  rgb_to_hsl: function rgb_to_hsl(r, g, b) {
    ...
    var min = Math.min(r, g, b),
        max = Math.max(r, g, b); // Calculating L component

    var l = 0.5 * (min + max); // Calculating S component
...
$(document).ready(function () {
  ChromeRuntimeAPI.get_background_context().then(function (context) {
    ...
    if (_typeof(session) === 'object') {
      ms_editor = new MSScreenshotEditorView();
      var ms_export = new MSFileExportPanel();
    } else {
...
function MSScreenshotEditorView() {
  ...
  self.state = {
    ...
    reset: function reset() {
      ...
      self.state.scale = 1; // Tool parameters
      ...
      var current_tool = global_settings.get('editor_tool');
      self.tools.select(!['crop', 'resize'].has(current_tool) ? current_tool : MSDefaultAppState.get('editor_tool'));
    },
    history: {
      reset: function reset() {
      ...
      get_current_head: function get_current_head() {
        return typeof self.state.history.head === 'number' ? self.state.history.head : self.state.history.stack.length ? self.state.history.stack.length - 1 : 0;
      },
      get: function get() {
      ...
      undo: function undo() {
        ...
        console.log("[History] Undo (Cursor:".concat(self.state.history.head, ")"));
      },
      redo: function redo() {
        ...
        console.log("[History] Redo (Cursor:".concat(self.state.history.head, ")"));
      },
      wipe: function wipe() {
  ...
  self.ui = {
    ...
    initialize: function initialize() {
      ...
      self.ui.$elements.tool_size_slider = new MSToolSizeSlider({
        ...
        on_change: function on_change(value) {
          self.state.tool_size = value;

          if (session.is_last()) {
            global_settings.set({
              editor_tool_size: value
            });
          }
        }
      });
    ...
    assign_event_handlers: function assign_event_handlers() {
      console.log('[Editor UI] Assigning event handlers'); // Hotkeys
          ...
          return regeneratorRuntime.wrap(function _callee$(_context) {
            while (1) {
              switch (_context.prev = _context.next) {
                ...
                case 4:
                  _context.next = 6;
                  return ms_editor.exportPNG();

                case 6:
                  data = _context.sent;
                  _context.next = 9;
                  return MSClipboard.writeImageBlobToClipboard(data);

                case 9:
                  self._dependencies.notifications.show({
                    message: chrome.i18n.getMessage('notification_image_copy_to_clipboard_success')
                  });
      ...
      $(window).resize(function () {// console.log(window.innerWidth, window.innerHeight);
      });
      ...
      $('#tool_color_picker').ColorPicker({
        ...
        onChange: function onChange(hsb, hex, rgb) {
          ...
          self.palette.pick_color("#".concat(hex));
        },
        onSubmit: function onSubmit() {
          self.palette.hide();
        }
      ...
      $(self.ui.$elements.canvas2).on('mousedown', function (e) {
        ...
        console.log("[Tool] ".concat(self.state.tool.capitalize(), ": ").concat(self.state.draw ? 'Finish' : 'Start', " drawing"));
        var canvas_offset = $(this).offset();
        var start_point = new Point(e.pageX - canvas_offset.left, e.pageY - canvas_offset.top);
        var rgb_color = MSColorTools.convert.hex_to_rgb(self.state.current_color);
        self.palette.hide();
        ...
        if (!e.button) {
          switch (self.state.tool) {
            ...
            case 'rectangle':
              ...
              self.state.draw = true;
              console.log("Highlight: ".concat(self.state.keys.list.alt));
      ...
      $(self.ui.$elements.canvas2).on('mousemove', function (e) {
        // console.log(`[Events] canvas2 ${e.type}`);
        var canvas_offset = $(this).offset();
        var current_point = new Point(e.pageX - canvas_offset.left, e.pageY - canvas_offset.top);

        if (self.state.draw) {
          if (['arrow', 'rectangle', 'line', 'ellipse', 'brush', 'blur'].has(self.state.tool) || self.state.tool === 'bubble' && self.state.current_primitive.arrow_in_progress) {
            self.scene.clear_canvas(this);
            ...
            switch (self.state.tool) {
              ...
              case 'blur':
                ...
                ctx.imageSmoothingEnabled = false; // Can be "low", "medium", or "high"
                // ctx.imageSmoothingQuality = 'low';

                ctx.drawImage(self.tools.blur.source_canvas, bounding_box.x / self.state.scale, bounding_box.y / self.state.scale, blur_width, blur_height, 0, 0, blur_width, blur_height); // TODO: There's an error while drawing blur too fast
          ...
          if (!e.button) {
            ...
            switch (self.state.tool) {
              ...
              case 'brush':
                ...
                if (self.state.current_primitive.bounding_box.diameter > self.settings.min_primitive_diameter) {
                  self.state.current_primitive.points = smooth_polyline_with_bezier_curves(optimize_polyline(self.state.current_primitive.points, 8, 0.8, 0.02), 30);
                  self.tools.finish_primitive_creation();
                } else {
                  delete self.state.current_primitive;
                }
      ...
      $(document).on('click', '#crop_cancel_button', self.tools.select_previous); // Resize UI
      ...
      $(document).on('click', '#resize_submit_button', self.tools.resize.submit); // Scaling UI
  ...
  self.palette = {
    default_colors: ['#e70404', '#044fe7', '#ff7300', '#000000', '#ffffff', '#e704c5', '#53e704', '#ffea00', '#8a8a8a'],
    is_shown: false,
    render: function render() {
      console.log('[Editor UI] Palette: render');
      $('#tool_settings_popup').html(self.ui.templates.palette.render({
        colors: self.palette.default_colors
      }));
    },
  ...
  self.tools = {
    ...
    select: function select(type) {
      console.log("[Tool] Select: ".concat(type));
      ...
      if (self.tools.types.has(type) && self.state.tool !== type) {
        ...
        $(".tool[data-action=".concat(type, "]")).addClass('active');
    ...
    get_bounding_box: function get_bounding_box(primitive) {
    ...
    scale_primitive: function scale_primitive(primitive, current_scale, target_scale, clamp_tool_size) {
      if (_typeof(primitive) === 'object' && primitive) {
        ...
        switch (primitive.type) {
          ...
          case 'bubble':
          case 'counter':
            primitive.points = primitive.points.map(function (p) {
    ...
    text: {
      start_timer: function start_timer() {
        self.state.text_timer = setInterval(function () {
      ...
      interrupt_input: function interrupt_input() {
        if (self.state.draw) {
          self.state.draw = false;
          self.tools.text.cancel();
          delete self.state.current_primitive;
    ...
    text_and_bubble: {
      detect_and_submit: function detect_and_submit() {
    ...
    blur: {
      ...
      prepare_source_data: function prepare_source_data() {
        console.log('[Scene] Blur: Source canvas preparing...');
        self.scene.export_image_data().then(function (output) {
          self.scene.scale_image(output.data, self.settings.blur_scale, self.settings.blur_scale).then(function (result) {
            var image_data = result.data,
                original_size = result.original_image.size;
            var ctx_b = self.tools.blur.source_canvas.getContext('2d');
    ...
    crop: {
      ...
      initialize: function initialize() {
        ...
        if (!self.tools.crop.image_crop_area) {
          ...
          self.tools.crop.image_crop_area.setOptions({
            enable: true
          });
        }
      ...
      submit: function submit() {
        ...
        self.scene.export_image_data({
          crop_area: self.tools.crop.selection
    ...
    resize: {
      ...
      set_new_image_size: function set_new_image_size(input_data, side, keep_proportion) {
        ...
        side = side || 'width';
        var other_side = side === 'width' ? 'height' : 'width';

        if (value) {
          value = parseInt(value).clamp(1, 50000); // console.log(value, value/self.tools.resize.original_size[side]);
      ...
      render_preview: function render_preview() {
      ...
      submit: function submit() {
        self.scene.scale_image(self.rendered_scene, self.state.current_primitive.width / self.tools.resize.original_size.width, self.state.current_primitive.height / self.tools.resize.original_size.height).then(function (output) {
          var image_data = output.data;
    ...
    finish_primitive_creation: function finish_primitive_creation() {
  ...
  self.scene = {
    zoom: function zoom() {
      ...
      var new_scale = options && typeof options.scale === 'number' && options.scale > 0 ? options.scale : get_the_nearest_in_a_sequence(self.state.scale, self.settings.editor_scale, options && options.direction === 'in' ? 'next' : 'prev');
    ...
    fit_image_to_window: function fit_image_to_window(image_data, callback) {
      var image = new Image();
      image.src = get_image_src(image_data);
    ...
    center_canvas: function center_canvas() {
    ...
    scale_image: function scale_image(image_data, scale_x, scale_y) {
      return new Promise(function (resolve, reject) {
        if (image_data) {
    ...
    export_image_data: function export_image_data() {
      var options = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
      return new Promise(function (resolve, reject) {
        ...
        var history = self.state.history.get()._copy();
        ...
        if (initial_source) {
          ...
          output.onload = function () {
            ...
            if (_typeof(options.crop_area) === 'object') {
              ...
              var c_ctx = cropped_canvas.getContext('2d');
              c_ctx.drawImage(buffer_canvas, options.crop_area.x, options.crop_area.y, options.crop_area.width, options.crop_area.height, 0, 0, options.crop_area.width, options.crop_area.height);
            }

            var output_canvas = cropped_canvas || buffer_canvas;
    ...
    render: function render() {
      ...
      return new Promise(function (resolve, reject) {
        ...
        var scale = typeof options.scale === 'number' && options.scale > 0 ? options.scale : self.state.scale;
        self.scene.export_image_data({
          scale: scale
        }).then(function (output) {
          ...
          $(self.ui.$elements.canvas2).show();
          self.tools.text.cancel();
          var rendered_canvas = output.canvas;
          ...
          var ctx = self.ui.$elements.canvas1.getContext('2d');
  ...
  self.exportPNG = function () {
    return self.scene.export_image_data({
      type: "png"
    }).then(function (output) {
      return output.data;
    });
...
/*#__PURE__*/
function () {
  function MSClipboard() {
    _classCallCheck(this, MSClipboard);
  }

  _createClass(MSClipboard, null, [{
    key: "writeImageBlobToClipboard",
    value: function writeImageBlobToClipboard(blob) {
...
function optimize_polyline(path, qLength, coefficient, cosine_coef) {
  ...
  var optimized_path = []; // Optimization: 1st pass
...
function smooth_polyline_with_bezier_curves(polyline_points, SepCoef) {
  ...
  for (var i = 1; i < initial_points_count - 1; ++i) {
    ...
    for (var j = 0; j < SepCoef; ++j) {
      var t = step * j;
      smooth_curve.push(new Point(Math.pow(1.0 - t, 2) * p0.x + 2 * (1.0 - t) * t * p1.x + Math.pow(t, 2) * p2.x, Math.pow(1.0 - t, 2) * p0.y + 2 * (1.0 - t) * t * p1.y + Math.pow(t, 2) * p2.y));
    }
  }
...
function compute_arrow_gradient_colors(color, r, dr) {
  var original_l = color.l;
  var new_l;
  var upper_limit = 100 - r + dr,
      lower_limit = r + dr;
...
function draw_arrow(canvas, color, width, p1, p2, reversed, shadow) {
  ...
  var k1 = new Point(begin.x, begin.y + 1);
  var k2 = new Point(end.x - 2.25 * h / tan, end.y + 1.25 * h);
  var k3 = new Point(k2.x - 4 * h * cos / 3, k2.y + 4 * h * sin / 3);
  var k4 = new Point(k3.x + 0.5 * diameter * sin, k3.y + 0.5 * diameter * cos);
  var k5 = new Point(k3.x + diameter * sin, k3.y + diameter * cos);
  var k6 = new Point(end.x, end.y);
  var k11 = new Point(begin.x, begin.y - 1);
  var k10 = new Point(end.x - 2.25 * h / tan, end.y - 1.25 * h);
  var k9 = new Point(k10.x - 4 * h * cos / 3, k10.y - 4 * h * sin / 3);
  var k8 = new Point(k9.x + 0.5 * diameter * sin, k9.y - 0.5 * diameter * cos);
  var k7 = new Point(k9.x + diameter * sin, k9.y - diameter * cos);
  ...
  var lighter_shade = new Point(end.x, end.y - 2.0 * width),
      darker_shade = new Point(end.x, end.y + 2.0 * width);
  var arrowGradient = arrow_angle < -pi / 2 ? ctx.createLinearGradient(darker_shade.x, darker_shade.y, lighter_shade.x, lighter_shade.y) : ctx.createLinearGradient(lighter_shade.x, lighter_shade.y, darker_shade.x, darker_shade.y);
...
function draw_rectangle(canvas, x, y, width, height, color, stroke_width, filled, highlighted, shadow) {
  ...
  if (highlighted) {
    ...
    var m_ctx = mask_canvas.getContext('2d');
...
function Text(width, color, position, placement, stroke, shadow, font_family, font_units) {
  ...
  this.draw = function (canvas) {
    ...
    if (!this.saved) {
      ...
      this.caret_animation.step = this.caret_animation.step < this.caret_animation.limit - 1 ? this.caret_animation.step + 1 : 0; // Rendering bounding box

      var ctx = canvas.getContext('2d');
      ctx.save();
      set_text_style_and_position(ctx, this.alignment, this.font_family, this.font_size, this.font_size_units, this.color); // TODO: Calculate bounding box on demand (when text is changed)

      this.bounding_box = get_text_bounding_box(ctx, this.text, this.current_position.x, this.current_position.y, this.alignment, this.line_spacing, this.stroke); // console.log(ctx.textBaseline, ctx.textAlign, this.bounding_box);
...
function Bubble(width, color, points, r, dr, reversed, stroke, shadow, font_family, font_units) {
  ...
  this.stroke = typeof stroke === 'boolean' ? stroke : true;
  this.shadow = typeof shadow === 'boolean' ? shadow : true; // Text properties
  ...
  this.show_bounding_box = true;
  this.bounding_box_dash_offset = 0; // Arrow properties
  ...
  this.draw = function (canvas) {
    canvas = canvas || ms_editor.ui.$elements.canvas1; // Rendering
    // 1. Text
    ...
    if (!this.arrow_in_progress) {
      ...
      if (!this.saved) {
        ...
        this.text_bounding_box = get_text_bounding_box(ctx, this.text, this.text_current_position.x, this.text_current_position.y, this.alignment, this.line_spacing, this.stroke); // console.log(ctx.textBaseline, ctx.textAlign, this.bounding_box);
...
function set_text_style_and_position(ctx, alignment, font_family, font_size, font_size_units, color) {
  ctx.font = "".concat(font_size).concat(font_size_units, " ").concat(font_family);
...
function draw_text_selection(canvas, selection, text_top_left_corner, text_alignment, text, selection_color, font_family, font_size, font_size_units, line_spacing, stroke) {
  if (selection.start !== selection.end) {
    var get_line_vertical_offset = function get_line_vertical_offset(line_number) {
      return line_spacing * line_number;
    };

    var render_selection = function render_selection(line, start, end, x, y, text_alignment) {
      var line_to_render = typeof end === 'number' ? line.substring(start, end) : line.substring(start); // console.log(line_to_render, start, end);
    ...
    var start_caret = get_caret_position_from_text_cursor(selection.start, text),
        end_caret = get_caret_position_from_text_cursor(selection.end, text); // console.log(selection, start_caret, end_caret);
    ...
    if (start_caret.line === end_caret.line) {
      ...
      // First line
      line_vert_offset = get_line_vertical_offset(start_caret.line - 1);
      render_selection(text_lines[start_caret.line - 1], start_caret.column, null, text_top_left_corner.x, text_top_left_corner.y + line_vert_offset + dy, text_alignment); // In between lines
...
function draw_text_caret(canvas, text_top_left_corner, text_alignment, text, caret, font_family, font_size, font_size_units, line_spacing) {
  ...
  var vert_line_offset = line_spacing * (caret.line - 1); // console.log(last_line, pre_caret_offset, caret);
...
function draw_accents(canvas, points, color, size, font_family, font_size_units, shadow) {
  var color_string = MSColorTools.generate_css.rgba(color.r, color.g, color.b);
  var font_size = 4 * size;
  var font_color = 255 - color.r < 20 && 255 - color.g < 20 && 255 - color.b < 20 ? {
    r: 0,
    g: 0,
    b: 0
  } : {
  ...
  var circle_radius = 1.4 * 0.5 * font_size;
  var ctx = canvas.getContext('2d');
  ctx.save();
...
function MSFileExportPanel() {
  ...
  self.ui = {
    ...
    initialize: function initialize() {
      ...
      self.ui.$elements.file_type_dropdown_trigger.text('.' + bg.ms_core.file.type_specific_info.extension.get(session.get_type()));
      self.ui.assign_event_handlers();
    },
    assign_event_handlers: function assign_event_handlers() {
      ...
      $(document).on('click', '.ms_file_format', function () {
        ...
        self.ui.$elements.file_type_dropdown_trigger.text('.' + bg.ms_core.file.type_specific_info.extension.get(session.get_type()));
        $(document).trigger('output_type_changed', {
      ...
      self.ui.$elements.file_save_button.on('click', function () {
        ...
        if (typeof ms_editor !== 'undefined') {
          ms_editor["export"]().then(function (data) {
            return session.save();
          });
        } else {
          session.save();
      ...
      self.ui.$elements.copy_to_clipboard_button.on('click',
      /*#__PURE__*/
      _asyncToGenerator(
      /*#__PURE__*/
...
function MSUploadBurger() {
  ...
  function _initialize() {
    _dependencies.transmitter.subscribe({
      'account_updated': function account_updated(request, callback) {
        console.log("[Editor] Burger menu: update (".concat(request.service, ")"));
  ...
  self.ui = {
    ...
    assign_event_handlers: function assign_event_handlers() {
      ...
      $(document).on('click', '.upload_to', function (e) {
        ...
        if (typeof ms_editor !== 'undefined') {
          ms_editor["export"]().then(function (data) {
            return self.upload(service, destination);
          });
        } else {
    ...
    render_destinations_list: function render_destinations_list($container, class_name, service, targets_list, id_key, name_key, select_after) {
      if ($container) {
        if (Array.isArray(targets_list) && targets_list.length) {
          ...
          targets_list.forEach(function (f) {
            ...
            $container.append("<li class=\"".concat(class_name, "\" data-service=\"").concat(service, "\" data-destination=\"").concat(id, "\"><a tabindex=\"-1\" href=\"#\">").concat(name, "</a></li>"));
          });
          select_after = typeof select_after === 'string' ? select_after : targets_list[0][id_key];
          $(".".concat(class_name, "[data-id=").concat(select_after, "]")).trigger('click');
        } else {
  ...
  self.services = {
    monosnap: {
      ...
      folder: {
        get: function get() {
          self.services.monosnap.account.folder.get_list().then(self.services.monosnap.folder.render)["catch"](function (data) {
            self.ui.$elements.monosnap_folders_list.addClass('ms_hidden');
          });
        },
        render: function render(folders) {
