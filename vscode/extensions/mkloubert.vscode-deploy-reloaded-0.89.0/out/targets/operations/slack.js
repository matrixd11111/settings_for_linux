...
async function execute(context) {
    ...
    const CHANNELS = Enumerable.from(deploy_helpers.asArray(OPERATION.channels)).select(c => {
        return deploy_helpers.toStringSafe(WORKSPACE.replaceWithValues(c)).toUpperCase()
            .trim();
    }).where(c => '' !== c)
        .distinct()
        .toArray();
    const TARGET_NAME = deploy_targets.getTargetName(context.target);
    const TOKEN = deploy_helpers.toStringSafe(WORKSPACE.replaceWithValues(OPERATION.token)).trim();
    const USER = deploy_helpers.normalizeString(WORKSPACE.replaceWithValues(OPERATION.user));
    let minimumNumberOfFiles = parseInt(deploy_helpers.toStringSafe(OPERATION.minimumNumberOfFiles).trim());
