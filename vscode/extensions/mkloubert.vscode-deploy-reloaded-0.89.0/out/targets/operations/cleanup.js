...
async function execute(context) {
    ...
    let dirToCleanup = deploy_helpers.toStringSafe(WORKSPACE.replaceWithValues(OPERATION.dir));
    ...
    const CLEANUP_DIR = async (d, subDir) => {
        ...
        for (const SD of (await deploy_helpers.readDir(d))) {
            const FULL_PATH = Path.resolve(Path.join(d, SD));
            const RELATIVE_PATH = FULL_PATH.substr(dirToCleanup.length);
            const DOES_MATCH = deploy_helpers.checkIfDoesMatchByFileFilter(TO_MINIMATCH(RELATIVE_PATH), deploy_helpers.toMinimatchFileFilter({
                files: INCLUDE,
                exclude: EXCLUDE,
            }));
            if (!DOES_MATCH) {
