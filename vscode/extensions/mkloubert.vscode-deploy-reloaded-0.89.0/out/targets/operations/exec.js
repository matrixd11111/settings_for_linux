...
async function execute(context) {
    ...
    let cwd = deploy_helpers.toStringSafe(WORKSPACE.replaceWithValues(OPERATION.cwd));
    ...
    let maxBuffer = parseInt(deploy_helpers.toStringSafe(OPERATION.maxBuffer).trim());
    if (isNaN(maxBuffer)) {
        maxBuffer = undefined;
    }
    let timeout = parseInt(deploy_helpers.toStringSafe(OPERATION.timeout).trim());
    if (isNaN(timeout)) {
        timeout = undefined;
    }
