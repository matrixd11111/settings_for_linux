...
async function checkBeforeDeploy(target, plugin, files, mappingScopeDirs, cancelToken) {
    ...
    if (!deploy_plugins.canList(plugin, target)) {
        let selectedValue;
        const SELECTED_ITEM = await WORKSPACE.showWarningMessage(WORKSPACE.t('deploy.checkBeforeDeploy.notSupported', TARGET_NAME), {
    ...
    try {
        ...
        for (const F of files) {
            const NAME_AND_PATH = deploy_targets.getNameAndPathForFileDeployment(target, F, mappingScopeDirs);
        ...
        if (OLDER_FILES.length > 0) {
            let selectedValue;
            const SELECTED_ITEM = await WORKSPACE.showWarningMessage(WORKSPACE.t('deploy.checkBeforeDeploy.newerFilesFound', OLDER_FILES.length), {
                isCloseAffordance: true,
                title: WORKSPACE.t('no'),
                value: 0,
            }, {
                ...
                case 0:
                    {
                        ...
                        const TABLE_HEADER_LOCAL_FILE = HTML_ENCODER.encode(WORKSPACE.t('deploy.checkBeforeDeploy.report.localFile'));
                        const TABLE_HEADER_REMOTE_FILE = HTML_ENCODER.encode(WORKSPACE.t('deploy.checkBeforeDeploy.report.remoteFile'));
                        const TABLE_HEADER_LAST_CHANGE = HTML_ENCODER.encode(WORKSPACE.t('deploy.checkBeforeDeploy.report.lastChange'));
                        const TABLE_HEADER_SIZE = HTML_ENCODER.encode(WORKSPACE.t('deploy.checkBeforeDeploy.report.size'));
                        ...
                        for (const F of OLDER_FILES) {
                            ...
                            for (const RF of NEWER_FILE_LIST) {
                                ...
                                const TABLE_CELL_LOCAL_FILE_MTIME = HTML_ENCODER.encode(LOCAL_MTIME.format(WORKSPACE.t('time.dateTimeWithSeconds')));
                                ...
                                const TABLE_CELL_REMOTE_FILE_MTIME = HTML_ENCODER.encode(REMOTE_MTIME.format(WORKSPACE.t('time.dateTimeWithSeconds')));
 ...
 */
async function deployAllOpenFiles(workspaces) {
    ...
    if (DOCUMENTS.length < 1) {
        deploy_helpers.showWarningMessage(i18.t('editors.noOpen'));
        return;
    }
    const CREATE_FILE_LIST_RELOADER = (ws) => {
        return () => {
    ...
    for (const WS of workspaces) {
        ...
        const TARGET = await deploy_targets.showTargetQuickPick(WS.context.extension, WS.getUploadTargets(), {
            placeHolder: WS.t('workspaces.selectTarget', WS.name),
        });
        if (!TARGET) {
            continue;
        }
        const TARGET_NAME = deploy_targets.getTargetName(TARGET);
        try {
            await deploy_helpers.applyFuncFor(deployFilesTo, WS)(FILES, TARGET, RELOADER);
        }
        catch (e) {
            WS.showErrorMessage(WS.t('deploy.errors.operationToTargetFailed', TARGET_NAME, e));
        }
    }
}
exports.deployAllOpenFiles = deployAllOpenFiles;
/**
 * Deploys a files from a file list of the active text editor.
 *
 * @param {vscode.ExtensionContext} context The extension context.
 */
async function deployFileList(context) {
    const WORKSPACE = await deploy_workspaces.showWorkspaceQuickPick(context, deploy_workspaces.getAllWorkspaces(), {
        placeHolder: i18.t('workspaces.selectWorkspace'),
    });
    if (!WORKSPACE) {
        return;
    }
    await WORKSPACE.startDeploymentOfFilesFromActiveDocument(async (target, files) => {
        await deploy_helpers.applyFuncFor(deployFilesTo, target.__workspace)(files, target, () => files);
    });
}
exports.deployFileList = deployFileList;
/**
 ...
 */
async function deployFilesTo(files, target, fileListReloader) {
    const ME = this;
    await ME.deployQueue.add(async () => {
        await deploy_helpers.withProgress(async (progress) => {
            await deploy_helpers.applyFuncFor(deployFilesToWithProgress, ME)(progress, files, target, fileListReloader);
        }, {
...
exports.deployFilesTo = deployFilesTo;
async function deployFilesToWithProgress(progress, files, target, fileListReloader) {
    const ME = this;
    ...
    const PREPARE_CANCELLED = !deploy_helpers.toBooleanSafe(await deploy_targets.executePrepareTargetOperations({
    ...
    if (reloadFileList) {
        files = deploy_helpers.asArray(await Promise.resolve(fileListReloader()));
        NORMALIZE_FILE_LIST();
    }
    if (files.length < 1) {
        return;
    ...
    await ME.deactivateAutoDeployOperationsFor(async () => {
        await deploy_helpers.applyFuncFor(saveBeforeDeploy, ME)(target, files);
    });
    const TARGET_NAME = deploy_targets.getTargetName(target);
    ...
    if (false === transformer) {
        throw new Error(ME.t('targets.errors.couldNotLoadDataTransformer', TARGET_NAME));
    }
    transformer = deploy_transformers.toDataTransformerSafe(deploy_transformers.toPasswordTransformer(transformer, target));
    ...
    const GET_FILE_PATH_FOR_DISPLAY = (f) => {
        if (!IS_MULTI_ROOT) {
    ...
    try {
        ...
        while (PLUGINS.length > 0) {
            ...
            // "finished button"
            await ME.invokeForFinishedButton(deploy_contracts.DeployOperation.Deploy, (btn) => btn.hide());
            const UPDATE_FINISHED_BTN = async (err) => {
                await ME.invokeForFinishedButton(deploy_contracts.DeployOperation.Deploy, async (btn) => {
                    ...
                    btn.text = `${icon} ` +
                        ...
                        ME.t('deploy.finishedButton.text', OVERALL_WATCH.stop());
            ...
            try {
                ...
                if (files.length > 1) {
                    ME.output.appendLine(`🚀 ` + ME.t('deploy.startOperation', TARGET_NAME));
                }
                let watch;
                ...
                const FILES_TO_UPLOAD = deploy_helpers.from(files.map(f => {
                    const NAME_AND_PATH = deploy_targets.getNameAndPathForFileDeployment(target, f, MAPPING_SCOPE_DIRS);
                    if (false === NAME_AND_PATH) {
                        return null;
                    }
                    const LF = new deploy_plugins.LocalFileToUpload(ME, f, NAME_AND_PATH);
                    LF.onBeforeUpload = async function (destination) {
                        const NOW = deploy_helpers.now();
                        ...
                        const PROGRESS_MSG = `🚀 ` +
                            ...
                            ME.t('deploy.deployingFile', GET_FILE_PATH_FOR_DISPLAY(f), destination);
                        ME.output.append(`[${NOW.format(ME.t('time.timeWithSeconds'))}] ` +
                            PROGRESS_MSG + ' ');
                    ...
                    LF.onUploadCompleted = async (err) => {
                        if (err) {
                    ...
                    LF.transformerSubContext = {
                        deployOperation: deploy_contracts.DeployOperation.Deploy,
                        file: LF.file,
                        remoteFile: deploy_helpers.normalizePath(NAME_AND_PATH.path + '/' + NAME_AND_PATH.name),
                        target: target,
                    };
                ...
                const SHOW_CANCELED_BY_OPERATIONS_MESSAGE = () => {
                    ME.output.appendLine(`✖️ ` + ME.t('deploy.canceledByOperation', TARGET_NAME));
                };
                let operationIndex;
                ...
                operationIndex = -1;
                ME.output.appendLine('');
                const BEFORE_DEPLOY_ABORTED = !deploy_helpers.toBooleanSafe(await deploy_targets.executeTargetOperations({
                    cancellationToken: progress.cancellationToken,
                    ...
                    onBeforeExecute: async (operation) => {
                        ++operationIndex;
                        ME.output.append(`⚡ ` + ME.t('targets.operations.runningBeforeDeploy', GET_OPERATION_NAME(operation)));
                        if (CANCELLATION_SOURCE.token.isCancellationRequested) {
                ...
                await Promise.resolve(PI.uploadFiles(CTX));
                // deployed
                operationIndex = -1;
                const AFTER_DEPLOY_ABORTED = !deploy_helpers.toBooleanSafe(await deploy_targets.executeTargetOperations({
                    cancellationToken: progress.cancellationToken,
                    files: FILES_TO_UPLOAD.map(ftu => {
                        return ftu.path + '/' + ftu.name;
                    }),
                    onBeforeExecute: async (operation) => {
                        ++operationIndex;
                        ME.output.append(`⚡ ` + ME.t('targets.operations.runningAfterDeployed', GET_OPERATION_NAME(operation)));
                        if (CANCELLATION_SOURCE.token.isCancellationRequested) {
                            ME.output.appendLine(`✖️`);
                        }
                        else {
                ...
                if (files.length > 1) {
                    ME.output.appendLine('');
                    ME.output.appendLine(`🚀 ` +
                        ME.t('deploy.finishedOperation', TARGET_NAME));
                }
                UPDATE_FINISHED_BTN(null);
            }
            catch (e) {
                ME.output.appendLine(`🔥 ` +
                    ME.t('deploy.finishedOperationWithErrors', TARGET_NAME, e));
                POPUP_STATS.failed = files;
                POPUP_STATS.succeeded = [];
                UPDATE_FINISHED_BTN(e);
            }
            finally {
                deploy_helpers.applyFuncFor(deploy_gui.showPopupWhenFinished, ME)(POPUP_STATS);
            }
        }
    }
    finally {
 ...
 * @param {deploy_targets.Target} target The target to deploy to.
 */
async function deployFileTo(file, target) {
    const ME = this;
    if (ME.isInFinalizeState) {
        return;
    }
    ...
    if (!ME.canBeHandledByMe(target)) {
        throw new Error(ME.t('deploy.errors.invalidWorkspace', file, ME.name));
    }
    file = Path.resolve(file);
    await deploy_helpers.applyFuncFor(deployFilesTo, ME)([file], target, null);
}
exports.deployFileTo = deployFileTo;
/**
 * Deploys a package.
 *
 ...
 */
async function deployPackage(pkg, targetResolver) {
    const ME = this;
    if (ME.isInFinalizeState) {
        return;
    }
    ...
    try {
        ...
        if (!ME.canBeHandledByMe(pkg)) {
            throw new Error(ME.t('pull.errors.invalidWorkspaceForPackage', deploy_packages.getPackageName(pkg), ME.name));
        }
        const RELOADER = async () => {
            const FILES_FROM_FILTER = await ME.findFilesByFilter(deploy_packages.preparePackageForFileFilter(pkg));
            await deploy_packages.importPackageFilesFromGit(pkg, deploy_contracts.DeployOperation.Deploy, FILES_FROM_FILTER);
            return FILES_FROM_FILTER;
        };
        const FILES_TO_DEPLOY = await RELOADER();
        if (FILES_TO_DEPLOY.length < 1) {
            ME.showWarningMessage(ME.t('noFiles'));
            return;
        }
        const TARGETS = deploy_helpers.applyFuncFor(deploy_packages.getTargetsOfPackage, ME)(pkg, targetResolver);
        if (false === TARGETS) {
            return;
        }
        const SELECTED_TARGET = await deploy_targets.showTargetQuickPick(ME.context.extension, TARGETS.filter(t => deploy_targets.isVisibleForPackage(t, pkg)), {
            placeHolder: ME.t('deploy.selectTarget'),
        });
        if (!SELECTED_TARGET) {
            return;
        }
        await deploy_helpers.applyFuncFor(deployFilesTo, ME)(FILES_TO_DEPLOY, SELECTED_TARGET, RELOADER);
    }
    finally {
        if (PACKAGE_BTN) {
            PACKAGE_BTN.show();
 ...
 */
async function deployOnChange(file) {
    ...
    try {
        const OPTS = {
            ...
            fastFileCheckFlagResolver: (pkg) => {
                return deploy_packages.getFastFileCheckFlag(pkg, (p) => p.fastCheckOnChange, CFG, (c) => c.fastCheckOnChange);
            },
        ...
        await deploy_helpers.applyFuncFor(deploy_packages.autoDeployFile, WORKSPACE)(OPTS);
    }
    finally {
        await FINISHED();
    }
}
exports.deployOnChange = deployOnChange;
/**
 * Deploys a file when is has been saved.
 *
 * @param {string} file The file to check.
 */
async function deployOnSave(file) {
    const WORKSPACE = this;
    if (WORKSPACE.isInFinalizeState) {
        return;
    }
    ...
    try {
        const OPTS = {
            ...
            fastFileCheckFlagResolver: (pkg) => {
                return deploy_packages.getFastFileCheckFlag(pkg, (p) => p.fastCheckOnSave, CFG, (c) => c.fastCheckOnSave);
            },
...
exports.deployOnSave = deployOnSave;
/**
 ...
 * @param {deploy_targets.Target} target The target to deploy to.
 */
async function deployScmCommit(client, target) {
    if (!client) {
        return;
    }
    const ME = this;
    ...
    const STORAGE_KEY = Crypto.createHash('sha256')
        .update(branch)
        .digest('hex');
    let filterStorage = ME.state.get(KEY_SCM_COMMIT_FILE_FILTERS);
    if (!_.isPlainObject(filterStorage)) {
        filterStorage = {};
    }
    let fileFilters = filterStorage[STORAGE_KEY];
    ...
    try {
        ...
        for (const C of CHANGES) {
            ...
            if (!deploy_helpers.checkIfDoesMatchByFileFilter('/' + FILE, deploy_helpers.toMinimatchFileFilter(fileFilters))) {
                continue; // does not match pattern(s)
            }
            const FULL_PATH = Path.resolve(Path.join(ME.rootPath, FILE));
            switch (C.type) {
        ...
        if (FILES_TO_DELETE.length > 0) {
            await deploy_helpers.applyFuncFor(deploy_delete.deleteFilesIn, ME)(FILES_TO_DELETE, target, null, false);
        }
        // then upload files
        if (FILES_TO_UPLOAD.length > 0) {
            await deploy_helpers.applyFuncFor(deployFilesTo, ME)(FILES_TO_UPLOAD, target, null);
        }
    }
    finally {
...
exports.deployScmCommit = deployScmCommit;
/**
 ...
 * @param {deploy_targets.Target} target The target to deploy to.
 */
async function deployUncommitedScmChanges(client, target) {
    if (!client) {
        return;
    }
    const ME = this;
    ...
    for (const C of CHANGES) {
        ...
        const FULL_PATH = Path.resolve(Path.join(client.cwd, FILE));
        switch (C.type) {
            case deploy_scm.FileChangeType.Added:
            case deploy_scm.FileChangeType.Modified:
                FILES_TO_UPLOAD.push(FULL_PATH);
...
exports.deployUncommitedScmChanges = deployUncommitedScmChanges;
/**
...
function registerDeployCommands(context) {
    ...
    vscode.commands.registerCommand('extension.deploy.reloaded.deploy', async () => {
        try {
            ...
            deploy_helpers.showErrorMessage(i18.t('deploy.errors.operationFailed'));
        }
    }), 
    // deploy file list
    ...
    vscode.commands.registerCommand('extension.deploy.reloaded.deployWorkspace', async () => {
        try {
            const PKG = await deploy_packages.showPackageQuickPick(context, deploy_packages.getAllPackagesSorted(), {
    ...
    vscode.commands.registerCommand('extension.deploy.reloaded.deployFile', async () => {
        try {
            await deploy_targets.invokeForActiveEditorAndTarget(i18.t('targets.selectTarget'), async (file, target) => {
    ...
    vscode.commands.registerCommand('extension.deploy.reloaded.deployAllOpenFiles', async () => {
        try {
            await deployAllOpenFiles(deploy_workspaces.getActiveWorkspaces());
        }
        catch (e) {
            deploy_log.CONSOLE
    ...
    vscode.commands.registerCommand('extension.deploy.reloaded.deployGitCommit', async () => {
        try {
            const WORKSPACE = await deploy_workspaces.showWorkspaceQuickPick(context, deploy_workspaces.getActiveWorkspaces(), {
                placeHolder: i18.t('workspaces.selectWorkspace')
            });
            if (!WORKSPACE) {
                return;
    ...
    vscode.commands.registerCommand('extension.deploy.reloaded.deployPackageFiles', async (packageNames, targetNames) => {
        ...
        for (const PN of packageNames) {
            ...
            for (const MP of MATCHING_PACKAGES) {
                try {
                    await deploy_helpers.applyFuncFor(deployPackage, MP.__workspace)(MP, () => targetNames);
                }
                catch (e) {
                    deploy_log.CONSOLE
...
async function saveBeforeDeploy(target, files) {
    const ME = this;
    const CFG = ME.config;
    if (!CFG) {
        return;
