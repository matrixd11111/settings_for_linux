...
async function compile(compileOpts) {
    ...
    const FILES_TO_COMPILE = await deploy_compilers.collectFiles(compileOpts, '**/*.pug');
    ...
    for (const FTC of FILES_TO_COMPILE) {
        ...
        try {
            ...
            const HTML = Pug.render((await deploy_helpers.readFile(FTC)).toString(enc), PUG_OPTS);
            await deploy_helpers.writeFile(OUTPUT_FILE, new Buffer(HTML, enc));
        }
        catch (e) {
            msg = {
