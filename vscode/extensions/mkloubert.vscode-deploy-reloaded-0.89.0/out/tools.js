...
async function createDeployScript(workspaces) {
    ...
    if (!Path.isAbsolute(scriptFile)) {
        scriptFile = Path.resolve(Path.join(scopeDir, scriptFile));
    }
    // find unique name
    let doesExist;
    let index = 0;
    do {
        ...
        if (doesExist) {
            ...
            scriptFile = Path.resolve(Path.join(DIR, `${BASENAME}-${index}${EXT}`));
        }
    } while (doesExist);
...
`;
    await deploy_helpers.writeFile(scriptFile, new Buffer(scriptContent, 'utf8'));
    try {
        const ASK_IF_WRITE_TO_SETTINGS_ITEMS = [
            {
        ...
        const SELECTED_ITEM = await selectedWorkspace.showWarningMessage
            .apply(selectedWorkspace, [selectedWorkspace.t('tools.createDeployScript.askForUpdatingSettings')].concat(ASK_IF_WRITE_TO_SETTINGS_ITEMS));
        if (SELECTED_ITEM) {
            if (1 === SELECTED_ITEM.value) {
                ...
                do {
                    ...
                    if (targetExists) {
                        selectedWorkspace.showWarningMessage(selectedWorkspace.t('tools.createDeployScript.targetAlreadyDefined'));
                    }
                } while (targetExists);
                if (!deploy_helpers.isEmptyString(newTargetName)) {
                    const CFG = selectedWorkspace.config;
                    const TARGETS_FROM_CFG = deploy_helpers.asArray(CFG.targets);
                    let targetScriptPath = scriptFile;
                    if (scriptFile.startsWith(selectedWorkspace.settingFolder)) {
                        targetScriptPath = './' + deploy_helpers.normalizePath(scriptFile.substr(selectedWorkspace.settingFolder.length));
                    }
                    ...
                    CLONED_CFG.targets = deploy_helpers.cloneObject(TARGETS_FROM_CFG);
                    const SETTINGS_FILE = Path.resolve(selectedWorkspace.configSource.resource.fsPath);
                    const SETTINGS_SECTION = selectedWorkspace.configSource.section;
                    let settings;
                    if (await deploy_helpers.exists(SETTINGS_FILE)) {
                        settings = JSON.parse((await deploy_helpers.readFile(SETTINGS_FILE)).toString('utf8'));
                    }
                    if (deploy_helpers.isNullOrUndefined(settings)) {
                        settings = {};
                    }
                    settings[SETTINGS_SECTION] = CLONED_CFG;
                    await deploy_helpers.writeFile(SETTINGS_FILE, new Buffer(JSON.stringify(settings, null, 4)));
                }
            }
        }
    }
    catch (e) {
        selectedWorkspace.showWarningMessage(selectedWorkspace.t('tools.createDeployScript.errors.updateTargetSettingsFailed', e));
    }
    selectedWorkspace.showInformationMessage(selectedWorkspace.t('tools.createDeployScript.scriptCreated', scriptFile));
    await vscode.window.showTextDocument(await vscode.workspace.openTextDocument(scriptFile));
}
exports.createDeployScript = createDeployScript;
/**
 ...
 * @param {deploy_workspaces.Workspace|deploy_workspaces.Workspace[]} workspaces One or more workspaces.
 */
async function createDeployOperationScript(context, workspaces) {
    workspaces = deploy_helpers.asArray(workspaces);
    const QUICK_PICKS = workspaces.map(ws => {
        return {
            label: ws.name,
    ...
    let scriptFile;
    let operationEvent;
    const OPERATION_TYPE_QUICK_PICKS = [
        {
    ...
    const SELECTED_OPERATION_TYPE_QUICK_PICK = await vscode.window.showQuickPick(OPERATION_TYPE_QUICK_PICKS, {
...
`;
    ...
    if (WORKSPACE_TARGETS.length > 0) {
        try {
            ...
            const SELECTED_ITEM = await selectedWorkspace.showWarningMessage
                .apply(selectedWorkspace, [selectedWorkspace.t('tools.createDeployOperationScript.askForUpdatingSettings')].concat(ASK_IF_WRITE_TO_SETTINGS_ITEMS));
            if (SELECTED_ITEM) {
                if (1 === SELECTED_ITEM.value) {
                    const selectedTarget = await deploy_targets.showTargetQuickPick(context, WORKSPACE_TARGETS, {
                    ...
                    if (selectedTarget) {
                        ...
                        const SETTINGS_FILE = Path.resolve(selectedWorkspace.configSource.resource.fsPath);
                        const SETTINGS_SECTION = selectedWorkspace.configSource.section;
                        const CLONED_TARGETS_FROM_CFG = deploy_helpers.asArray(CLONED_CFG.targets);
                        let targetScriptPath = scriptFile;
                        if (scriptFile.startsWith(selectedWorkspace.settingFolder)) {
                            targetScriptPath = './' + deploy_helpers.normalizePath(scriptFile.substr(selectedWorkspace.settingFolder.length));
                        }
                        let operationStorage = false;
                        let updater;
                        ...
                        if (TARGET_ITEM_FROM_CLONED_CFG) {
                            switch (operationEvent) {
                                case deploy_targets.TargetOperationEvent.AfterDeployed:
                                    ...
                                    updater = () => {
                                        TARGET_ITEM_FROM_CLONED_CFG.deployed = operationStorage;
                                    };
                                    break;
                                case deploy_targets.TargetOperationEvent.BeforeDeploy:
                                    ...
                                    updater = () => {
                                        TARGET_ITEM_FROM_CLONED_CFG.beforeDeploy = operationStorage;
                                    };
                                    break;
                            }
                        }
            ...
            selectedWorkspace.showWarningMessage(selectedWorkspace.t('tools.createDeployOperationScript.errors.updateSettingsFailed', e));
        }
    }
    selectedWorkspace.showInformationMessage(selectedWorkspace.t('tools.createDeployOperationScript.scriptCreated', scriptFile));
    await vscode.window.showTextDocument(await vscode.workspace.openTextDocument(scriptFile));
}
exports.createDeployOperationScript = createDeployOperationScript;
/**
 * Detects changes between two git commits.
 *
 * @param {vscode.ExtensionContext} context The underlying extension context.
 */
async function detectGitChanges(context) {
    const SELECT_WORKSPACE = await deploy_workspaces.showWorkspaceQuickPick(context, deploy_workspaces.getAllWorkspaces());
    if (!SELECT_WORKSPACE) {
        return;
    }
    ...
    await deploy_helpers.withProgress(async (progress) => {
        ...
        do {
            ...
            ++page;
            run = true;
            progress.message = SELECT_WORKSPACE.t('scm.loadingCommits', BRANCH.id, page);
        ...
        for (let i = 0; i < COMMIT_WINDOW.length; i++) {
            ...
            progress.message = SELECT_WORKSPACE.t('scm.loadingCommitChanges', C.id, i + 1, COMMIT_WINDOW.length);
        ...
        for (const FILE in ALL_CHANGES) {
            ...
            const FULL_PATH = Path.resolve(GIT_FOLDER, '..', CHG.file);
        ...
        if (_.isEmpty(ADDED_FILES) && _.isEmpty(DELETED_FILES) && _.isEmpty(MODIFIED_FILES)) {
            SELECT_WORKSPACE.showWarningMessage(SELECT_WORKSPACE.t('scm.changes.noneFound'));
            return;
        }
        ...
        if (!progress.cancellationToken.isCancellationRequested) {
            await EDITOR.edit((builder) => {
                builder.insert(new vscode.Position(0, 0), text.trim());
            });
        }
    });
}
exports.detectGitChanges = detectGitChanges;
/**
 * Resets the package usage statistics.
 *
 * @param {vscode.ExtensionContext} context The extension context.
 */
function resetToolUsage(context) {
    context.workspaceState.update(exports.KEY_TOOL_USAGE, undefined).then(() => {
    }, (err) => {
        deploy_log.CONSOLE
            .trace(err, 'tools.resetToolUsage()');
    });
}
exports.resetToolUsage = resetToolUsage;
/**
 ...
 */
async function showPackageFiles(context, packages) {
    const PACKAGE = await deploy_packages.showPackageQuickPick(context, packages, {
    ...
    await deploy_packages.importPackageFilesFromGit(PACKAGE, deploy_contracts.DeployOperation.Deploy, files);
    await deploy_packages.importPackageFilesFromGit(PACKAGE, deploy_contracts.DeployOperation.Pull, files);
    await deploy_packages.importPackageFilesFromGit(PACKAGE, deploy_contracts.DeployOperation.Delete, files);
    ...
    if (files.length > 0) {
        ...
        for (const F of files) {
            const FULL_PATH = Path.join(WORKSPACE.rootPath, F);
            let deployOnSave = '';
            {
                let deployOnSaveTargets = await deploy_helpers.applyFuncFor(deploy_packages.findTargetsForFileOfPackage, WORKSPACE)(FULL_PATH, () => PACKAGE.deployOnSave, () => {
                    return deploy_packages.getFastFileCheckFlag(PACKAGE, (p) => p.fastCheckOnSave, WORKSPACE.config, (c) => c.fastCheckOnSave);
                });
            ...
            let deployOnChange = '';
            {
                let deployOnChangeTargets = await deploy_helpers.applyFuncFor(deploy_packages.findTargetsForFileOfPackage, WORKSPACE)(FULL_PATH, () => PACKAGE.deployOnChange, () => {
                    return deploy_packages.getFastFileCheckFlag(PACKAGE, (p) => p.fastCheckOnChange, WORKSPACE.config, (c) => c.fastCheckOnChange);
                });
            ...
            let syncWhenOpen = '';
            {
                let syncWhenOpenTargets = await deploy_helpers.applyFuncFor(deploy_packages.findTargetsForFileOfPackage, WORKSPACE)(FULL_PATH, () => PACKAGE.syncWhenOpen, () => {
                    return deploy_packages.getFastFileCheckFlag(PACKAGE, (p) => p.fastCheckOnSync, WORKSPACE.config, (c) => c.fastCheckOnSync);
                });
            ...
            let removeOnChange = '';
            {
                let removeOnChangeTargets = await deploy_helpers.applyFuncFor(deploy_packages.findTargetsForFileOfPackage, WORKSPACE)(FULL_PATH, () => PACKAGE.removeOnChange, () => true);
    ...
    await deploy_html.openMarkdownDocument(md, {
        documentTitle: "[vscode-deploy-reloaded] " + WORKSPACE.t('tools.showPackageFiles.title', PACKAGE_NAME),
    });
}
exports.showPackageFiles = showPackageFiles;
