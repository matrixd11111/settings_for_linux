...
const deploy_compare = require("./compare");
...
const deploy_notifications = require("./notifications");
...
const deploy_tools = require("./tools");
const deploy_tools_bower = require("./tools/bower");
const deploy_tools_composer = require("./tools/composer");
const deploy_tools_npm = require("./tools/npm");
const deploy_tools_quick_execution = require("./tools/quickexecution");
const deploy_tools_send_file = require("./tools/sendfile");
const deploy_tools_yarn = require("./tools/yarn");
...
async function createNewWorkspace(folder) {
    let newWorkspace;
    let fileWatcher;
    try {
        fileWatcher = vscode.workspace.createFileSystemWatcher(new vscode.RelativePattern(folder, '**'), false, false, false);
        const CTX = {
        ...
        newWorkspace = new deploy_workspaces.Workspace(nextWorkspaceId--, folder, CTX);
        outputChannel.append(i18.t('workspaces.initializing', folder.uri.fsPath) + ' ');
        try {
            ...
            if (HAS_BEEN_INITIALIZED) {
                ...
                throw new Error(i18.t('workspaces.errors.notInitialized', folder.uri.fsPath));
            }
        }
        catch (err) {
...
function getAllTargetsSorted() {
    return Enumerable.from(getAllWorkspacesSorted()).selectMany(ws => {
        return Enumerable.from(ws.getTargets()).orderBy(t => {
            return deploy_helpers.normalizeString(deploy_targets.getTargetName(t));
        }).thenBy(t => {
            return t.__index;
        });
    }).toArray();
}
function normalizeActiveWorkspaces(aws) {
    aws = deploy_helpers.asArray(aws);
    if (aws.length < 1) {
        if (1 === deploy_workspaces.getAllWorkspaces().length) {
            aws = deploy_helpers.asArray(Enumerable.from(deploy_workspaces.getAllWorkspaces())
...
async function onDidSaveTextDocument(e) {
    if (isDeactivating) {
        return;
    }
    ...
    if (await deploy_helpers.exists(PLUGIN_DIR)) {
        ...
        if (STATS.isDirectory()) {
            ...
            if (JS_FILES.length > 0) {
                    ...
                    try {
                        ...
                        if (MODULE) {
                            ...
                            if (CREATE_PLUGINS) {
                                ...
                                const NEW_PLUGINS = deploy_helpers.asArray(await Promise.resolve(CREATE_PLUGINS.apply(MODULE, [CONTEXT])));
                                if (NEW_PLUGINS) {
                                    let index = -1;
                                    for (const PI of NEW_PLUGINS) {
                                        ...
                                        try {
                                            ...
                                            PI.__type = deploy_helpers.toStringSafe(Path.basename(JS, Path.extname(JS))).toLowerCase().trim();
                                            let isInitialized;
                                            const INITILIZE = PI.initialize;
                                            if (INITILIZE) {
                                                isInitialized =
                                                    await Promise.resolve(INITILIZE.apply(PI, []));
                                            }
                                            if (deploy_helpers.toBooleanSafe(isInitialized, true)) {
                                                PLUGINS.push(PI);
                                            }
                                            else {
                                                deploy_helpers.showErrorMessage(i18.t('plugins.errors.notInitialized', PI.__file));
                                            }
                                        }
                                        catch (e) {
                                            deploy_helpers.showErrorMessage(i18.t('plugins.errors.initializationFailed', JS));
                                            deploy_log.CONSOLE
                                ...
                                deploy_helpers.showWarningMessage(i18.t('plugins.errors.noFactoryFunction', JS));
                            }
                        }
                        else {
                            deploy_helpers.showWarningMessage(i18.t('plugins.errors.noModule', JS));
                        }
                    }
                    catch (e) {
                        deploy_helpers.showErrorMessage(i18.t('plugins.errors.loadingFailed', JS));
                        deploy_log.CONSOLE
                ...
                deploy_helpers.showErrorMessage(i18.t('plugins.errors.noneFoundIn', PLUGIN_DIR));
            }
        }
        else {
            deploy_helpers.showErrorMessage(i18.t('isNo.dir', PLUGIN_DIR));
        }
    }
    else {
        deploy_helpers.showErrorMessage(i18.t('notFound.dir', PLUGIN_DIR));
    }
}
...
async function activate(context) {
    ...
    WF.next(async () => {
        const VS_DEPLOY = Enumerable.from(vscode.extensions.all).firstOrDefault(x => 'mkloubert.vs-deploy' === x.id);
        let doActivateTheExtension = true;
        if (!deploy_helpers.isSymbol(VS_DEPLOY)) {
            if (VS_DEPLOY.isActive) {
                doActivateTheExtension = false;
                const PRESSED_BTN = await deploy_helpers.showWarningMessage(i18.t('vs-deploy.currentlyActive'), {
        ...
        if (doActivateTheExtension) {
            ...
            deploy_helpers.showInformationMessage(i18.t('initializationCanceled'));
        }
    });
    if (!isDeactivating) {
        await WF.start();
...
async function activateExtension(context) {
    ...
    // events
    WF.next(() => {
        context.subscriptions.push(deploy_helpers.EVENT_DISPOSER);
    });
    // output channel
    WF.next(() => {
        context.subscriptions.push(outputChannel = vscode.window.createOutputChannel('Deploy Reloaded'));
        outputChannel.hide();
    });
    // workspace (folders)
    WF.next(() => {
        context.subscriptions.push(workspaceWatcher = deploy_helpers.registerWorkspaceWatcher(context, async (ev, folder) => {
            ...
            if (ev === deploy_helpers.WorkspaceWatcherEvent.Removed) {
                const NEW_ACTIVE_WORKSPACES = deploy_helpers.asArray(activeWorkspaces).filter(aws => aws !== workspace);
    ...
    WF.next(async () => {
        try {
            await deploy_helpers.createDirectoryIfNeeded(deploy_helpers.getExtensionDirInHome());
        }
        catch (e) {
            deploy_log.CONSOLE
    ...
    WF.next(() => {
        ...
        vscode.commands.registerCommand('extension.deploy.reloaded.compare', async () => {
            try {
                const QUICK_PICKS = [
                    {
                        action: async () => {
                            await deploy_compare.compareFiles(deploy_workspaces.getActiveWorkspaces());
                        },
                ...
                deploy_helpers.showErrorMessage(i18.t('compare.errors.operationFailed'));
            }
        }), 
        // handle current file(s) or folder(s)
        vscode.commands.registerCommand('extension.deploy.reloaded.currentFileOrFolder', async function (fileOrFolder, allItems) {
            await deploy_commands.handleFilesAndFolders(context, fileOrFolder, allItems);
        }), 
        // list directory
        vscode.commands.registerCommand('extension.deploy.reloaded.listDirectory', async () => {
            try {
                const TARGET = await deploy_targets.showTargetQuickPick(context, getAllTargetsSorted(), {
                ...
                deploy_helpers.showErrorMessage(i18.t('listDirectory.errors.operationFailed'));
            }
        }), 
        // API (hosts)
        vscode.commands.registerCommand('extension.deploy.reloaded.apis', async () => {
            try {
                ...
                vscode.window.showErrorMessage(i18.t('apis.errors.failed', e));
            }
        }), 
        // (TCP) proxies
        vscode.commands.registerCommand('extension.deploy.reloaded.proxies', async () => {
            try {
                ...
                vscode.window.showErrorMessage(i18.t('proxies.errors.failed', e));
            }
        }), 
        // quick code execution
        vscode.commands.registerCommand('extension.deploy.reloaded.quickExecution', async () => {
            try {
                await deploy_tools_quick_execution._1b87f2ee_b636_45b6_807c_0e2d25384b02_1409614337(context, getAllWorkspacesSorted(), activeWorkspaces.map(aws => aws));
            }
            catch (e) {
                vscode.window.showErrorMessage(i18.t('tools.quickExecution.errors.failed', e));
            }
        }), 
        // select workspace
        vscode.commands.registerCommand('extension.deploy.reloaded.selectWorkspace', async () => {
            try {
                ...
                deploy_helpers.showErrorMessage(i18.t('workspaces.active.errors.selectWorkspaceFailed'));
            }
            finally {
                await updateActiveWorkspaces();
            }
        ...
        vscode.commands.registerCommand('extension.deploy.reloaded.switches', async () => {
            try {
                const QUICK_PICKS = [
                    {
                        action: async () => {
                            await deploy_switch.changeSwitch(getActiveWorkspacesOrAll());
                        },
                ...
                deploy_helpers.showErrorMessage(i18.t('switches.errors.operationFailed'));
            }
        }), 
        // tools
        vscode.commands.registerCommand('extension.deploy.reloaded.showTools', async () => {
            try {
                const QUICK_PICKS = [
                    ...
                    {
                        action: async () => {
                            await deploy_tools.createDeployScript(getActiveWorkspacesOrAll());
                        },
                    ...
                    {
                        action: async () => {
                            await deploy_tools.createDeployOperationScript(context, getActiveWorkspacesOrAll());
                        },
                    ...
                    {
                        action: async () => {
                            await deploy_tools.showPackageFiles(context, deploy_packages.getAllPackagesSorted());
                        },
                ...
                const SELECTED_ITEM = await vscode.window.showQuickPick(deploy_gui.sortQuickPicksByUsage(QUICK_PICKS, context.workspaceState, deploy_tools.KEY_TOOL_USAGE, (i) => {
                    // remove icon
                    return i.label
    ...
    WF.next(async () => {
        ...
        let update = true;
        try {
            const LAST_VERSION = deploy_helpers.normalizeString(context.globalState.get(STATE_KEY, ''));
            if ('' === LAST_VERSION) {
                return;
            }
    ...
    WF.next(() => {
        try {
            outputChannel.appendLine(i18.t('network.hostname', OS.hostname()));
    ...
    WF.next(() => {
        context.subscriptions.push(deploy_proxies.PROXY_DISPOSER);
    });
    WF.next(() => {
        outputChannel.appendLine('');
    ...
    WF.next(() => {
        try {
            deploy_notifications.showExtensionNotifications(context, packageFile).then(() => {
            }, (err) => {
                deploy_log.CONSOLE
