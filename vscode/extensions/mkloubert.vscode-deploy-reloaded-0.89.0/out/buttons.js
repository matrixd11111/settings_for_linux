...
function createFinishedButton(state, key, id) {
...
exports.disposeFinishedButtons = disposeFinishedButtons;
/**
 ...
 */
function getFinishedButton(operation) {
    const WORKSPACE = this;
    const STATE = WORKSPACE.workspaceSessionState;
    if (STATE) {
        const BUTTONS = STATE['buttons'];
...
exports.getFinishedButton = getFinishedButton;
/**
 ...
 */
function initFinishedButtons(state) {
    const WORKSPACE = this;
    const ID = nextFinishedBtnIds++;
    // dispose old
    deploy_helpers.applyFuncFor(disposeFinishedButtons, WORKSPACE)();
    state['timeouts'][KEY_FINISHED_BTNS] = {};
    // deploy
    state['buttons'][KEY_FINISHED_BTN_DEPLOY] = deploy_helpers.applyFuncFor(createFinishedButton, WORKSPACE)(state, KEY_FINISHED_BTN_DEPLOY, ID);
    // delete
    state['buttons'][KEY_FINISHED_BTN_DELETE] = deploy_helpers.applyFuncFor(createFinishedButton, WORKSPACE)(state, KEY_FINISHED_BTN_DELETE, ID);
    // pull
    state['buttons'][KEY_FINISHED_BTN_PULL] = deploy_helpers.applyFuncFor(createFinishedButton, WORKSPACE)(state, KEY_FINISHED_BTN_PULL, ID);
}
exports.initFinishedButtons = initFinishedButtons;
/**
        ...
        try {
            btn = await deploy_helpers.createButton(b, async (nb, btnDesc) => {
                ...
                cmd = vscode.commands.registerCommand(BTN_CMD_ID, async function () {
                    try {
                        return await Promise.resolve(vscode.commands.executeCommand
                            .apply(null, [REAL_CMD].concat(await GET_ARGUMENTS())));
                    }
                    catch (e) {
                        deploy_log.CONSOLE
                            .trace(e, `${BTN_CMD_ID}(${i}.'${REAL_CMD}')`);
                        WORKSPACE.showErrorMessage(WORKSPACE.t('error', e));
                    }
                });
 ...
 */
function setTimeoutForFinishedButton(operation, callback, ms = 60000) {
    const WORKSPACE = this;
    const STATE = WORKSPACE.workspaceSessionState;
    if (STATE) {
        let timeouts;
        ...
        if (timeouts) {
            ...
            if (false !== key) {
                deploy_helpers.tryDisposeAndDelete(timeouts, key);
                const BTN = deploy_helpers.applyFuncFor(getFinishedButton, WORKSPACE)(operation);
                if (BTN) {
...
exports.setTimeoutForFinishedButton = setTimeoutForFinishedButton;
