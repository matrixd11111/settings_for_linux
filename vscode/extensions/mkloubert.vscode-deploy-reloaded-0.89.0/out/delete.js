 ...
 */
async function deleteFileIn(file, target, askForDeleteLocalFile = true) {
    const ME = this;
    ...
    if (!ME.canBeHandledByMe(target)) {
        throw new Error(ME.t('DELETE.errors.invalidWorkspace', file, target.__workspace.name));
    }
    const BUTTONS = [
        {
            title: ME.t('no'),
    ...
    if (deploy_helpers.toBooleanSafe(askForDeleteLocalFile, true)) {
        const PRESSED_BTN = await vscode.window.showWarningMessage.apply(null, [ME.t('DELETE.askIfDeleteLocalFile'), {}].concat(BUTTONS));
        if (!PRESSED_BTN || 0 == PRESSED_BTN.value) {
            return;
        }
        deleteLocalFile = 2 === PRESSED_BTN.value;
    }
    await deploy_helpers.applyFuncFor(deleteFilesIn, ME)([file], target, null, deleteLocalFile);
}
exports.deleteFileIn = deleteFileIn;
/**
 * Deletes a files from a file list of the active text editor.
 *
 * @param {vscode.ExtensionContext} context The extension context.
 */
async function deleteFileList(context) {
    ...
    const PRESSED_BTN = await vscode.window.showWarningMessage.apply(null, [WORKSPACE.t('DELETE.askIfDeleteLocalFiles'), {}].concat(BUTTONS));
    if (!PRESSED_BTN || 0 == PRESSED_BTN.value) {
        return;
    }
    const DELETE_LOCAL_FILES = 2 === PRESSED_BTN.value;
    await WORKSPACE.startDeploymentOfFilesFromActiveDocument(async (target, files) => {
        await deploy_helpers.applyFuncFor(deleteFilesIn, target.__workspace)(files, target, () => files, DELETE_LOCAL_FILES);
    });
}
 ...
 */
async function deleteFilesIn(files, target, fileListReloader, deleteLocalFiles) {
    const ME = this;
    await ME.deployQueue.add(async () => {
        await deploy_helpers.withProgress(async (progress) => {
            await deploy_helpers.applyFuncFor(deleteFilesInWithProgress, ME)(progress, files, target, fileListReloader, deleteLocalFiles);
        }, {
...
exports.deleteFilesIn = deleteFilesIn;
async function deleteFilesInWithProgress(progress, files, target, fileListReloader, deleteLocalFiles) {
    const ME = this;
    ...
    try {
        ...
        while (PLUGINS.length > 0) {
            ...
            // "finished button"
            await ME.invokeForFinishedButton(deploy_contracts.DeployOperation.Delete, (btn) => btn.hide());
            const UPDATE_FINISHED_BTN = async (err) => {
                await ME.invokeForFinishedButton(deploy_contracts.DeployOperation.Delete, async (btn) => {
                    ...
                    btn.text = `${icon} ` +
                        ...
                        ME.t('DELETE.finishedButton.text', OVERALL_WATCH.stop());
            ...
            try {
                ...
                if (files.length > 1) {
                    ME.output.appendLine(`🗑️ ` + ME.t('DELETE.startOperation', TARGET_NAME));
                }
                let watch;
                ...
                const FILES_TO_DELETE = deploy_helpers.from(files.map(f => {
                    ...
                    SF.onBeforeDelete = async function (destination) {
                        const NOW = deploy_helpers.now();
                        ...
                        const PROGRESS_MSG = `🗑️ ` +
                            ...
                            ME.t('DELETE.deletingFile', GET_FILE_PATH_FOR_DISPLAY(f), destination);
                    ...
                    SF.onDeleteCompleted = async (err, deleteLocal) => {
                        if (err) {
                ...
                const SHOW_CANCELED_BY_OPERATIONS_MESSAGE = () => {
                    ME.output.appendLine(`✖️ ` + ME.t('DELETE.canceledByOperation', TARGET_NAME));
                };
                let operationIndex;
                ...
                operationIndex = -1;
                ME.output.appendLine('');
                const BEFORE_DELETE_ABORTED = !deploy_helpers.toBooleanSafe(await deploy_targets.executeTargetOperations({
                    cancellationToken: progress.cancellationToken,
                    ...
                    onBeforeExecute: async (operation) => {
                        ++operationIndex;
                        ME.output.append(`⚡ ` + ME.t('targets.operations.runningBeforeDelete', GET_OPERATION_NAME(operation)));
                        if (CANCELLATION_SOURCE.token.isCancellationRequested) {
                            ME.output.appendLine(`✖️`);
                        }
                        else {
                ...
                await Promise.resolve(PI.deleteFiles(CTX));
                // deleted
                operationIndex = -1;
                const AFTER_DELETED_ABORTED = !deploy_helpers.toBooleanSafe(await deploy_targets.executeTargetOperations({
                    cancellationToken: progress.cancellationToken,
                    files: FILES_TO_DELETE.map(ftu => {
                        return ftu.path + '/' + ftu.name;
                    }),
                    onBeforeExecute: async (operation) => {
                        ++operationIndex;
                        ME.output.append(`⚡ ` + ME.t('targets.operations.runningAfterDeleted', GET_OPERATION_NAME(operation)));
                        if (CANCELLATION_SOURCE.token.isCancellationRequested) {
                            ME.output.appendLine(`✖️`);
                        }
                        else {
                ...
                if (files.length > 1) {
                    ME.output.appendLine('');
                    ME.output.appendLine(`🗑️ ` +
                        ME.t('DELETE.finishedOperation', TARGET_NAME));
                }
                UPDATE_FINISHED_BTN(null);
            }
            catch (e) {
                ME.output.appendLine(`🔥 ` +
                    ME.t('DELETE.finishedOperationWithErrors', TARGET_NAME, e));
 ...
 */
async function deletePackage(pkg, targetResolver, askForDeleteLocalFiles = true) {
    const ME = this;
    if (ME.isInFinalizeState) {
        return;
    }
    ...
    if (!ME.canBeHandledByMe(pkg)) {
        throw new Error(ME.t('DELETE.errors.invalidWorkspaceForPackage', deploy_packages.getPackageName(pkg), ME.name));
    }
    ...
    const RELOADER = async () => {
        ...
        await deploy_packages.importPackageFilesFromGit(pkg, deploy_contracts.DeployOperation.Delete, FILES_FROM_FILTER);
    ...
    if (deploy_helpers.toBooleanSafe(askForDeleteLocalFiles, true)) {
        const PRESSED_BTN = await vscode.window.showWarningMessage.apply(null, [ME.t('DELETE.askIfDeleteLocalFiles'), {}].concat(BUTTONS));
        if (!PRESSED_BTN || 0 == PRESSED_BTN.value) {
            return;
        }
    ...
    await deploy_helpers.applyFuncFor(deleteFilesIn, ME)(FILES_TO_DELETE, SELECTED_TARGET, RELOADER, deleteLocalFiles);
}
 ...
 * @param {vscode.ExtensionContext} context The extension context.
 */
function registerDeleteCommands(context) {
    ...
    vscode.commands.registerCommand('extension.deploy.reloaded.delete', async () => {
        try {
            ...
            deploy_helpers.showErrorMessage(i18.t('DELETE.errors.operationFailed'));
        }
    }), 
    // delete file list
    ...
    vscode.commands.registerCommand('extension.deploy.reloaded.deletePackageFiles', async (packageNames, targetNames) => {
        ...
        for (const PN of packageNames) {
            ...
            for (const MP of MATCHING_PACKAGES) {
                try {
                    await deploy_helpers.applyFuncFor(deletePackage, MP.__workspace)(MP, () => targetNames);
                }
                catch (e) {
                    deploy_log.CONSOLE
...
exports.registerDeleteCommands = registerDeleteCommands;
