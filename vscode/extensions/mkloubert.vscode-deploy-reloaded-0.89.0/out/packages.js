 ...
 */
exports.KEY_PAUSE_FILES_FOR = 'pauseFilesFor';
 ...
 */
async function autoDeployFile(opts) {
    const WORKSPACE = this;
    try {
        const TARGETS = await deploy_helpers.applyFuncFor(findTargetsForFileOfPackage, WORKSPACE)(opts.file, opts.settingsResolver, opts.fastFileCheckFlagResolver, opts.prepareForPackage);
        if (false === TARGETS) {
            return;
        }
        for (const T of deploy_helpers.from(TARGETS).distinct(true)) {
            await invokeForAutoTargetOperation(T, async (target) => {
                ...
                try {
                    ...
                    WORKSPACE.showErrorMessage(WORKSPACE.t(opts.errorMsgTemplate, opts.file, TARGET_NAME, e));
                }
            });
        }
    }
...
exports.autoDeployFile = autoDeployFile;
/**
 ...
 */
async function findTargetsForFileOfPackage(file, settingsResolver, fastFileCheckFlagResolver, prepareForPackage) {
    const ME = this;
    const CFG = ME.config;
    ...
    for (const PKG of ME.getPackages()) {
        if (prepareForPackage) {
            await Promise.resolve(prepareForPackage(PKG));
        }
        let settings;
        if (settingsResolver) {
            settings = await Promise.resolve(settingsResolver(PKG));
        }
        ...
        const MATCHING_TARGETS = deploy_targets.getTargetsByName(targetNames, KNOWN_TARGETS);
        if (false === MATCHING_TARGETS) {
            return false;
        }
        const FAST_FILE_CHECK = deploy_helpers.toBooleanSafe(await Promise.resolve(fastFileCheckFlagResolver(PKG)));
        let fileListResolver;
        if (FAST_FILE_CHECK) {
            fileListResolver = () => {
                const FILE_LIST = [];
                const REL_PATH = ME.toRelativePath(file);
                if (false !== REL_PATH) {
                    const DOES_MATCH = deploy_helpers.checkIfDoesMatchByFileFilter('/' + REL_PATH, deploy_helpers.toMinimatchFileFilter(filter));
                    if (DOES_MATCH) {
                        FILE_LIST.push(file);
                    }
                }
            ...
            fileListResolver = async () => {
                let fileList;
                if (useFastGlob) {
                    fileList = (await ME.findFilesByFilterFast(PKG)).map((x) => {
                ...
                if (filter !== PKG) {
                    fileList = fileList.filter(f => {
                        ...
                        if (false !== REL_PATH) {
                            return deploy_helpers.checkIfDoesMatchByFileFilter('/' + REL_PATH, deploy_helpers.toMinimatchFileFilter(filter));
                        }
                        return false;
                    });
                }
        ...
        const FILE_LIST = deploy_helpers.asArray(await Promise.resolve(fileListResolver(filter, file)));
...
exports.findTargetsForFileOfPackage = findTargetsForFileOfPackage;
/**
...
function getAllPackagesSorted() {
    return deploy_helpers.from(deploy_workspaces.getAllWorkspaces()).orderBy(ws => {
        ...
        return Enumerable.from(ws.getPackages()).orderBy(pkg => {
            return deploy_helpers.normalizeString(getPackageName(pkg));
        }).thenBy(pkg => {
            return pkg.__index;
        });
    }).toArray();
}
exports.getAllPackagesSorted = getAllPackagesSorted;
/**
 ...
 */
function getFastFileCheckFlag(obj, flagResolver, parentObj, parentFlagResolver) {
    if (!obj) {
        obj = {};
    }
    if (!flagResolver) {
    ...
    const FAST_FILE_CHECK = deploy_helpers.toBooleanSafe(flagResolver(obj), deploy_helpers.toBooleanSafe(obj.fastFileCheck));
    const PARENT_FAST_FILE_CHECK = deploy_helpers.toBooleanSafe(parentFlagResolver(parentObj), deploy_helpers.toBooleanSafe(parentObj.fastFileCheck));
    return deploy_helpers.toBooleanSafe(FAST_FILE_CHECK, PARENT_FAST_FILE_CHECK);
}
exports.getFastFileCheckFlag = getFastFileCheckFlag;
/**
...
function getPackageName(pkg) {
    ...
    const TRANSLATOR = pkg.__workspace;
    if (!pkg) {
        return;
    }
 ...
 */
function getPauseFilesForValue(pkg, currentValue) {
    if (pkg) {
        const NEW_VALUE = parseInt(deploy_helpers.toStringSafe(pkg.pauseFilesFor).trim());
...
exports.getPauseFilesForValue = getPauseFilesForValue;
/**
 ...
 */
function getTargetsOfPackage(pkg, targetResolver) {
    const ME = this;
    if (!pkg) {
        return;
    }
    ...
    if (false !== targets) {
        if (targets.length < 1) {
            ...
            const BATCH_TARGET = {
                ...
                __workspace: ME,
                name: ME.t('packages.virtualTarget', getPackageName(pkg)),
                type: 'batch',
...
exports.getTargetsOfPackage = getTargetsOfPackage;
async function invokeForAutoTargetOperation(target, action) {
    if (!target) {
        return;
    }
    ...
    try {
        ...
        if (action) {
            await Promise.resolve(action(target));
        }
    }
    finally {
 ...
 */
async function importPackageFilesFromGit(pkg, operation, files) {
    if (!files) {
        files = [];
    }
    if (!pkg) {
    ...
    if (!deploy_helpers.isObject(gitSettings)) {
    ...
    if (false === CLIENT) {
        throw new Error(WORKSPACE.t('workspaces.errors.cannotDetectGitClient', WORKSPACE.name));
    }
    let rootFolderForGitFiles = WORKSPACE.gitFolder;
    if (false === rootFolderForGitFiles) {
        throw new Error(WORKSPACE.t('workspaces.errors.cannotDetectGitFolder', WORKSPACE.name));
    }
    rootFolderForGitFiles = Path.resolve(Path.join(rootFolderForGitFiles, '..'));
    ...
    const BRANCH = Enumerable.from(await CLIENT.branches()).singleOrDefault(b => {
        return branchName === deploy_helpers.normalizeString(b.id);
    });
    if (deploy_helpers.isSymbol(BRANCH)) {
        throw new Error(WORKSPACE.t('workspaces.errors.cannotFindBranch', branchName, WORKSPACE.name));
    }
    const FIND_COMMIT = async (hash) => {
        hash = deploy_git.normalizeGitHash(hash);
        // take first hash
        let page = 0;
        do {
            ++page;
            const COMMITS = await BRANCH.commits(page);
            if (COMMITS.length < 1) {
                break;
            }
    ...
    if (null === FROM_HASH) {
        // latest
        const COMMITS = await BRANCH.commits();
        if (COMMITS.length > 0) {
    ...
    if (!firstCommit) {
        throw new Error(WORKSPACE.t('workspaces.errors.cannotFindScmHash', FROM_HASH, WORKSPACE.name));
    }
    let lastCommit = firstCommit;
    if (null !== TO_HASH) {
        lastCommit = await FIND_COMMIT(TO_HASH);
    }
    if (!lastCommit) {
        throw new Error(WORKSPACE.t('workspaces.errors.cannotFindScmHash', TO_HASH, WORKSPACE.name));
    }
    ...
    const DOES_FILE_MATCH = (f) => {
        ...
        return deploy_helpers.checkIfDoesMatchByFileFilter(TO_MINIMATCH(REL_PATH), {
            files: filePatterns,
...
exports.importPackageFilesFromGit = importPackageFilesFromGit;
/**
 ...
 */
function preparePackageForFileFilter(pkg) {
    if (_.isNil(pkg)) {
        return pkg;
    }
...
exports.preparePackageForFileFilter = preparePackageForFileFilter;
/**
 * Handles a file for "remove on change" feature.
 *
 * @param {string} file The file to check.
 */
async function removeOnChange(file) {
    const WORKSPACE = this;
    ...
    const FINISHED = async () => {
        deploy_helpers.tryDisposeAndDelete(STATE['auto'], exports.KEY_PAUSE_FILES_FOR);
        if (isNaN(pauseFilesFor)) {
            RESTORE_IN_PROGRESS_STATE();
        }
        else {
            STATE['auto'][exports.KEY_PAUSE_FILES_FOR] = deploy_helpers.createTimeout(() => {
                RESTORE_IN_PROGRESS_STATE();
            }, pauseFilesFor);
        }
    };
    try {
        const TARGETS = await deploy_helpers.applyFuncFor(findTargetsForFileOfPackage, WORKSPACE)(file, (pkg) => pkg.removeOnChange, (pkg) => true, (pkg) => {
        ...
        for (const T of deploy_helpers.from(TARGETS).distinct(true)) {
            await invokeForAutoTargetOperation(T, async (target) => {
                ...
                try {
                    ...
                    WORKSPACE.showErrorMessage(WORKSPACE.t('DELETE.onChange.failed', file, TARGET_NAME, e));
                }
            });
        }
    }
 ...
 */
function resetPackageUsage(context) {
...
exports.resetPackageUsage = resetPackageUsage;
/**
 ...
 */
async function showPackageQuickPick(context, packages, opts) {
    const QUICK_PICKS = deploy_helpers.asArray(packages).filter(pkg => {
    ...
    if (QUICK_PICKS.length < 1) {
        deploy_helpers.showWarningMessage(i18.t('packages.noneFound'));
        return false;
    }
    let selectedItem;
    if (1 === QUICK_PICKS.length) {
        ...
        selectedItem = await vscode.window.showQuickPick(deploy_gui.sortQuickPicksByUsage(QUICK_PICKS, context.workspaceState, KEY_PACKAGE_USAGE, (i) => {
            // remove icon
            return i.label
...
exports.showPackageQuickPick = showPackageQuickPick;
async function waitForOtherAutoTargetOperations(target) {
    if (!target) {
        return;
    }
    await deploy_helpers.waitWhile(() => {
        return !deploy_helpers.isNullOrUndefined(AUTO_DEPLOY_STATES[target.__id]);
    }, {
        timeout: 60000,
