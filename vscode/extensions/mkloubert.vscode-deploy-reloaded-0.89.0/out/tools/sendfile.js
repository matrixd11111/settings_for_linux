 ...
 */
exports.CLOSE_SERVER_COMMAND = "extension.deploy.reloaded.receiveFile.closeServer";
...
async function receiveFile(context) {
    ...
    const LAST_PORT = deploy_helpers.toStringSafe(context.globalState.get(LAST_PORT_KEY)).trim();
    const STR_PORT = await vscode.window.showInputBox({
        prompt: i18.t('tools.sendOrReceiveFile.receive.enterPort', DEFAULT_PORT),
        value: LAST_PORT,
    });
    if (deploy_helpers.isNullOrUndefined(STR_PORT)) {
        return;
    }
    let port = parseInt(STR_PORT.trim());
    if (isNaN(port)) {
        port = DEFAULT_PORT;
    }
    ...
    const SHOW_ERROR = (err) => {
        ...
        errorShown = true;
        server = null;
        deploy_helpers.showErrorMessage(i18.t('tools.sendOrReceiveFile.receive.errors.startHostFailed', err));
    };
    try {
        ...
        serverButton.command = exports.CLOSE_SERVER_COMMAND;
        serverButton.text = "Deploy Reloaded: " + i18.t('tools.sendOrReceiveFile.receive.button.text', port);
        ...
        server = await SimpleSocket.listen(port, (err, remoteConnection) => {
            ...
            const SHOW_RECEIVE_ERROR = (err) => {
                if (receiveErrorShown) {
                    return;
                }
                receiveErrorShown = true;
                deploy_helpers.showErrorMessage(i18.t('tools.sendOrReceiveFile.receive.errors.couldNotReceiveFile', err));
            };
            try {
                remoteConnection.readJSON().then((file) => {
                    try {
                        ...
                        deploy_helpers.invokeForTempFile(async (tmpFile) => {
                            try {
                                await deploy_helpers.writeFile(tmpFile, new Buffer(deploy_helpers.toStringSafe(file.content), 'utf8'));
 ...
 * @param {vscode.ExtensionContext} context The extension context.
 */
function registerSendFileCommands(context) {
    ...
    vscode.commands.registerCommand(exports.CLOSE_SERVER_COMMAND, () => {
        closeServer();
    }));
}
exports.registerSendFileCommands = registerSendFileCommands;
/**
 * Sends a file.
 *
 * @param {vscode.ExtensionContext} context The extension context.
...
async function sendFile(context) {
    ...
    if (ACTIVE_EDITOR) {
        ...
        if (DOC) {
            try {
                const LAST_REMOTE_ADDR = deploy_helpers.normalizeString(context.globalState.get(LAST_REMOTE_ADDR_KEY));
                const REMOTE_ADDR = deploy_helpers.normalizeString(await vscode.window.showInputBox({
                ...
                if (SEP > -1) {
                    ...
                    port = parseInt(REMOTE_ADDR.substr(SEP + 1).trim());
                }
                else {
                    host = REMOTE_ADDR;
                }
                ...
                deploy_helpers.showErrorMessage(i18.t('tools.sendOrReceiveFile.send.errors.couldNotSendFile', e));
            }
        }
        else {
 ...
 * @param {vscode.ExtensionContext} context The extension context.
 */
async function sendOrReceiveFile(context) {
    const QUICK_PICKS = [
        {
            action: async () => {
                await sendFile(context);
...
exports.sendOrReceiveFile = sendOrReceiveFile;
