 ...
 */
async function _1b87f2ee_b636_45b6_807c_0e2d25384b02_1409614337(extension, allWorkspaces, activeWorkspaces) {
    // vscode
    ...
    // all workspaces
    // tslint:disable-next-line:no-unused-variable
    const $w = $h.asArray(allWorkspaces).map(ws => {
    ...
    // active workspaces
    // tslint:disable-next-line:no-unused-variable
    const $aw = $h.asArray(activeWorkspaces).map(aws => {
    ...
    // tslint:disable-next-line:no-unused-variable
    const $unwrap = async (val, maxDepth, currentDepth) => {
        if (isNaN(maxDepth)) {
            maxDepth = 64;
        }
        ...
        if (currentDepth < maxDepth) {
            if (val) {
                if ('function' === typeof val) {
                    val = $unwrap(Promise.resolve(val()), maxDepth, currentDepth + 1);
                }
            }
        }
        return val;
    ...
    const $unwrapArgs = async (args) => {
        const UNWRAPPED_ARGS = [];
        args = await $unwrap(args);
        if (args) {
            for (let i = 0; i < args.length; i++) {
                UNWRAPPED_ARGS.push(await $unwrap(args[i]));
            }
        }
    ...
    // resolve()
    // tslint:disable-next-line:no-unused-variable
    const $res = async (val, ...funcs) => {
        val = await $unwrap(val);
        let lastResult = val;
        for (const F of funcs) {
            let result;
            if (F) {
                result = await Promise.resolve(F(lastResult));
            }
    ...
    // tslint:disable-next-line:no-unused-variable
    const $exec = async function (...funcs) {
        let lastResult;
        for (const F of funcs) {
            let result;
            if (F) {
    ...
    // tslint:disable-next-line:no-unused-variable
    const $s = async (val) => {
        return $h.toStringSafe(await $unwrap(val));
    };
    // eval()
    // tslint:disable-next-line:no-unused-variable
    const $e = async (code) => {
        return await $unwrap(eval(await $s(code)));
    };
    // toFullPath()
    // tslint:disable-next-line:no-unused-variable
    const $fp = async (p) => {
        const Path = require('path');
        p = $h.toStringSafe(await $unwrap(p));
        if (!Path.isAbsolute(p)) {
            p = Path.join($aw[0].rootPath, p);
        }
    ...
    // executeCommand()
    // tslint:disable-next-line:no-unused-variable
    const $c = async (id, ...cmdArgs) => {
        id = $h.toStringSafe(await $unwrap(id));
        cmdArgs = $h.asArray(await $unwrap(cmdArgs), false);
        const ARGS = [];
        if (cmdArgs) {
            for (const A of cmdArgs) {
                ARGS.push(await $unwrap(A));
            }
        }
        return await Promise.resolve($vs.commands.executeCommand
            .apply(null, [id].concat(ARGS)));
    };
    // tslint:disable-next-line:no-unused-variable
    const $cleanup = async (dir) => {
        ...
        dir = await $fp(dir);
        for (const F of (await $h.readDir(dir))) {
            await $fs.remove($p.resolve($p.join(dir, F)));
        }
    };
    // list commands
    // tslint:disable-next-line:no-unused-variable
    const $commands = async (alsoInternalCommands) => {
        alsoInternalCommands = $h.toBooleanSafe(await $unwrap(alsoInternalCommands));
        const VSCODE_COMMANDS = (await $vs.commands.getCommands(!alsoInternalCommands)).filter(c => {
        ...
        for (const GRP of COMMAND_GROUPS) {
            const HEADER = GRP[0];
            const COMMANDS = $linq.from(GRP[1]).orderBy(c => {
        ...
        await $html.openMarkdownDocument(md.trim(), '[vscode-deploy-reloaded] Visual Studio Code commands');
    };
    // tslint:disable-next-line:no-unused-variable
    const $emoji = async (key) => {
        key = $h.toStringSafe(await $unwrap(key));
    ...
    // tslint:disable-next-line:no-unused-variable
    const $emoji_name = async (e) => {
        e = $h.toStringSafe(await $unwrap(e));
    ...
    // tslint:disable-next-line:no-unused-variable
    const $guid = async (ver, ...guidArgs) => {
        ver = $h.normalizeString(await $unwrap(ver));
        guidArgs = $h.asArray(await $unwrap(guidArgs));
        return $h.guid
            .apply(null, [ver].concat(guidArgs));
    };
    // tslint:disable-next-line:no-unused-variable
    const $hash = async (algo, val, asBinary) => {
        algo = $h.normalizeString(await $unwrap(algo));
        if ('' === algo) {
            algo = 'sha256';
        }
        val = await $unwrap(val);
        ...
        asBinary = await $unwrap($h.toBooleanSafe(asBinary));
        const Crypto = require('crypto');
        const RESULT = Crypto.createHash(algo).update(await $h.asBuffer(val));
    ...
    // tslint:disable-next-line:no-unused-variable
    const $ip = async (v6 = false, timeout, useHttps = false) => {
        const PublicIP = require('public-ip');
        v6 = $h.toBooleanSafe(await $unwrap(v6));
        timeout = parseInt($h.toStringSafe(await $unwrap(v6)).trim());
        if (isNaN(timeout)) {
            timeout = 5000;
        }
        useHttps = $h.toBooleanSafe(await $unwrap(useHttps));
        const OPTS = {
            https: useHttps,
            timeout: timeout,
        };
    ...
    // tslint:disable-next-line:no-unused-variable    
    const $lines = async (patterns, exclude) => {
    ...
    // tslint:disable-next-line:no-unused-variable
    const $lower = async (val) => {
        return $h.toStringSafe(await $unwrap(val)).toLowerCase();
    };
    // tslint:disable-next-line:no-unused-variable
    const $md5 = async (val, asBinary) => {
        return await $hash('md5', val, asBinary);
    };
    // tslint:disable-next-line:no-unused-variable
...
`;
    ...
    // tslint:disable-next-line:no-unused-variable
    const $now = async (timeZone) => {
        const NOW = $m();
        timeZone = $h.toStringSafe(await $unwrap(timeZone)).trim();
        return '' === timeZone ? NOW
            : NOW.tz(timeZone);
    };
    // tslint:disable-next-line:no-unused-variable
    const $pwd = async (size = 20, chars = 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789') => {
        size = parseInt($h.toStringSafe(await $unwrap(size)).trim());
        if (isNaN(size)) {
            size = 20;
        }
        chars = $h.toStringSafe(await $unwrap(chars));
        let result = '';
        const BYTES = await $h.randomBytes(size * 4);
    ...
    // tslint:disable-next-line:no-unused-variable
    const $rand = async function (minOrMax, max) {
        const RandomFloat = require('random-float');
        minOrMax = parseFloat($h.toStringSafe(await $unwrap(minOrMax)).trim());
        max = parseFloat($h.toStringSafe(await $unwrap(max)).trim());
        const ARGS = [];
        if (!isNaN(max)) {
    ...
    // tslint:disable-next-line:no-unused-variable
    const $rf = async (file) => {
        return await $h.readFile(await $fp(file));
    };
    // tslint:disable-next-line:no-unused-variable
    const $run = function () {
        let activeDocument;
        ...
        if (!activeDocument) {
            throw new Error($i18.t('editors.active.noOpen'));
        }
        return $e(activeDocument.getText());
    };
    // tslint:disable-next-line:no-unused-variable
    const $sha1 = async (val, asBinary) => {
        return await $hash('sha1', val, asBinary);
    };
    // tslint:disable-next-line:no-unused-variable
    const $sha256 = async (val, asBinary) => {
        return await $hash('sha256', val, asBinary);
    };
    // tslint:disable-next-line:no-unused-variable
    const $ltrim = async (val) => {
        return $h.toStringSafe(await $unwrap(val)).replace(/^\s+/, '');
    };
    // tslint:disable-next-line:no-unused-variable
    const $trim = async (val) => {
        return $h.toStringSafe(await $unwrap(val)).trim();
    };
    // tslint:disable-next-line:no-unused-variable
    const $rtrim = async (val) => {
        return $h.toStringSafe(await $unwrap(val)).replace(/\s+$/, '');
    };
    // tslint:disable-next-line:no-unused-variable
    const $upper = async (val) => {
        return $h.toStringSafe(await $unwrap(val)).toUpperCase();
    };
    // tslint:disable-next-line:no-unused-variable
    const $utc = async () => {
        return $m.utc();
    ...
    // tslint:disable-next-line:no-unused-variable
    const $uuid = async function (ver, ...args) {
    ...
    // writeFile()
    // tslint:disable-next-line:no-unused-variable
    const $wf = async (file, data, enc) => {
        data = await $h.asBuffer(await $unwrap(data), await $unwrap(enc));
        await $h.writeFile(await $fp(file), data);
    };
    // show help
    // tslint:disable-next-line:no-unused-variable
    const $help = async () => {
        await require('../html').openMarkdownDocument(_27adf674_b653_4ee0_a33d_4f60be7859d2(), {
    ...
    extension.workspaceState.update('vscdrLastQuickExecutionCode', _def303d6_7db1_4511_8365_e93ed7979b92_1379012881);
    allWorkspaces = undefined;
    activeWorkspaces = undefined;
    extension = undefined;
    const RESULT = await Promise.resolve($e(_def303d6_7db1_4511_8365_e93ed7979b92_1379012881));
    let resultToDisplay = RESULT;
    let displayer;
    if (!_.isUndefined(RESULT)) {
        const HtmlEntities = require('html-entities');
        const HTML_ENC = new HtmlEntities.AllHtmlEntities();
        const GET_TYPE_OF = (val) => {
            let type;
            if (!_.isNil(val)) {
        ...
        displayer = () => {
            $vs.window.showInformationMessage($h.toStringSafe(resultToDisplay)).then(() => { }, (err) => {
        ...
        if (Buffer.isBuffer(resultToDisplay)) {
            ...
            html += '</html>';
            displayer = () => {
                require('../html').openHtmlDocument(html, '[vscode-deploy-reloaded] ' + $i18.t('tools.quickExecution.result.title'));
            };
        }
        else if (_.isArray(resultToDisplay) || (resultToDisplay[Symbol.iterator] === 'function') || $linq.isEnumerable(resultToDisplay)) {
            const ITEMS = $linq.from(resultToDisplay).toArray();
            ...
            md = md.trim();
            displayer = () => {
                require('../html').openMarkdownDocument(md, {
    ...
    if (displayer) {
        await Promise.resolve(displayer());
    }
}
exports._1b87f2ee_b636_45b6_807c_0e2d25384b02_1409614337 = _1b87f2ee_b636_45b6_807c_0e2d25384b02_1409614337;
...
//# sourceMappingURL=quickexecution.js.map