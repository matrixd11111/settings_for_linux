 ...
 */
async function syncDocumentWhenOpen(doc) {
    const ME = this;
    ...
    const DIR = ME.toRelativePath(Path.dirname(FILE));
    if (false === DIR) {
        return;
    }
    ...
    const TARGETS = await deploy_helpers.applyFuncFor(deploy_packages.findTargetsForFileOfPackage, ME)(FILE, (pkg) => pkg.syncWhenOpen, (pkg) => {
        return deploy_packages.getFastFileCheckFlag(pkg, (p) => p.fastCheckOnSync, ME.config, (c) => c.fastCheckOnSync);
    });
    if (false === TARGETS) {
        return;
    }
