...
class SFTPPlugin extends deploy_plugins.AsyncFileClientPluginBase {
    constructor() {
        super(...arguments);
    ...
    async createContext(target) {
        ...
        let privateKeyFile = this.replaceWithValues(target, target.privateKey);
        ...
        if (false === privateKeyFile) {
            throw new Error(i18.t('sftp.privateKeyNotFound', target.privateKey));
        }
        ...
        try {
            const CTX = {
                client: await deploy_clients_sftp.openConnection({
                    ...
                    readyTimeout: parseInt(deploy_helpers.toStringSafe(this.replaceWithValues(target, target.readyTimeout)).trim()),
