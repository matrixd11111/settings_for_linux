...
class EachPlugin extends deploy_plugins.IterablePluginBase {
    async prepareTarget(eachTarget, target) {
        const ME = this;
        const WORKSPACE = eachTarget.__workspace;
        const CLONED_TARGETS = [];
        let from = eachTarget.from;
        if (deploy_helpers.isString(from)) {
            // extrenal source
            const DOWNLOAD_SOURCE = ME.replaceWithValues(eachTarget, from);
            from = JSON.parse((await deploy_download.download(DOWNLOAD_SOURCE, WORKSPACE.getSettingScopes())).toString('utf8'));
        }
