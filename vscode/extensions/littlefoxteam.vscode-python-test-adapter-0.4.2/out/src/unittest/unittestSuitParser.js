...
function parseTestSuites(content, cwd) {
    ...
    const allTests = (discoveryResult.tests || [])
        ...
        .filter(id => id)
        .map(id => id);
    const aggregatedErrors = Array.from(utilities_1.groupBy((discoveryResult.errors || []), e => e.class).entries())
    ...
    const discoveryErrorSuites = aggregatedErrors.map(({ id, file }) => ({
        type: 'test',
        id: id.testId,
        file,
        label: id.testLabel,
    }));
    const suites = Array.from(utilities_1.groupBy(allTests, t => t.suiteId).entries())
        .map(([suiteId, tests]) => {
